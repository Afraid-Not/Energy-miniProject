...
print("Test 컬럼 수:", len(test.columns))
...
y = train['일조(hr)']
test_1 = test[available_sunshine_features].fillna(0)
...
# 테스트 예측
xgb_test_pred = xgb_model.predict(test_1)
lgb_test_pred = lgb_model.predict(test_1)
test_pred = xgb_weight * xgb_test_pred + lgb_weight * lgb_test_pred

print(f"\n=== 일조 예측값 범위 ===")
print(f"XGBoost: {xgb_test_pred.min():.4f} ~ {xgb_test_pred.max():.4f}")
print(f"LightGBM: {lgb_test_pred.min():.4f} ~ {lgb_test_pred.max():.4f}")
print(f"앙상블: {test_pred.min():.4f} ~ {test_pred.max():.4f}")
...
# 개별 모델 성능
xgb_solar_smape = smape(y_solar_val, xgb_solar_val_pred)
lgb_solar_smape = smape(y_solar_val, lgb_solar_val_pred)
xgb_solar_r2 = r2_score(y_solar_val, xgb_solar_val_pred)
lgb_solar_r2 = r2_score(y_solar_val, lgb_solar_val_pred)
...
# 앙상블 검증 예측
solar_ensemble_val_pred = xgb_solar_weight * xgb_solar_val_pred + lgb_solar_weight * lgb_solar_val_pred
solar_ensemble_smape = smape(y_solar_val, solar_ensemble_val_pred)
solar_ensemble_r2 = r2_score(y_solar_val, solar_ensemble_val_pred)
...
# 파일 저장
try:
    train.to_csv(trainer + 'new_train_ver2.csv', index=False)
    test.to_csv(trainer + 'new_test_ver2.csv', index=False)
...
print(f"\n=== 최종 성능 요약 ===")
print(f"일조 앙상블 SMAPE: {ensemble_smape:.4f}")
print(f"일사량 앙상블 SMAPE: {solar_ensemble_smape:.4f}")
print("두 모델의 장점을 결합한 안정적인 예측 완료! 🎯")