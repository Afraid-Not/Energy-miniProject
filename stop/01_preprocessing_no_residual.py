...
data_path = './Energy/'
csv_path = './Energy/stop/'
trainer = './Energy/stop/trainer/'
save_path = './Energy/stop/submission/'
os.makedirs(save_path, exist_ok=True)
os.makedirs(trainer, exist_ok=True)

seed_file = "./Energy/stop/(SEED_COUNT)06_preprocessing.json"
...
def predict_with_ensemble_enhanced(models_dict, X_test, is_insolation=False):
    ...
    for i in range(len(ensemble_pred)):
        ...
        if is_transition:  # 전환 구간 (최우선)
            ensemble_pred[i] = (xgb_pred[i] * 0.1 + lgbm_pred[i] * 0.1 + 
                               decline_pred[i] * 0.1 + peak_pred[i] * 0.1 + 
                               transition_pred[i] * 0.5 + cat_pred[i] * 0.1)
...
# 결과 저장
train_all.to_csv(trainer + f'06_train_{SEED}_2.csv', index=False)
test_all.to_csv(trainer + f'06_test_{SEED}_2.csv', index=False)
