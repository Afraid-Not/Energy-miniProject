...
N_TRIALS = 5
SAMPLE_SIZE = 0.1
SAMPLE_SIZE_BNUM = 0.2
AUGMENTATION = 20

print(f"[Variable]")
print(f"N_SPLITS {N_SPLITS} | N_TRIALS {N_TRIALS} | AUGMENTATION {1+(60/AUGMENTATION)} times")
print(f"SAMPLE_SIZE {SAMPLE_SIZE} | SAMPLE_SIZE_BNUM {SAMPLE_SIZE_BNUM}\n")
...
def _train_one_model_cv_optuna(   # ← 이름 유지(호환)
    ...
    # === winsorizing 옵션 ===
    use_winsor=True,
    win_group_cols=('building_num','hour'),
    win_q_low=None,
    win_q_high=0.995,
    win_clip_lower=False,
    win_clip_upper=True,
    win_min_samples=30,
):
    """TSS + fold-train만 상한 윈저 → Optuna로 XGB/LGB 하이퍼 튜닝 → best 하이퍼로 최종 재학습"""
    # -------------------- 내부 유틸 --------------------
    def _winsor_train_part(df_part, series, group_cols, q_low, q_high, clip_lower, clip_upper, min_samples):
        y = series.copy()
        if not group_cols:
            lo = float(np.nanquantile(y, q_low)) if q_low is not None else None
            hi = float(np.nanquantile(y, q_high)) if q_high is not None else None
            if clip_lower and lo is not None and np.isfinite(lo): y = y.mask(y < lo, lo)
            if clip_upper and hi is not None and np.isfinite(hi): y = y.mask(y > hi, hi)
            return y
        if isinstance(group_cols, tuple):
            group_cols = list(group_cols)
        caps_hi = (
            df_part.groupby(group_cols)[target_col]
                  .transform(lambda s: s.quantile(q_high) if (q_high is not None and len(s) >= min_samples) else np.nan)
            if q_high is not None else pd.Series(np.nan, index=df_part.index)
        )
        caps_lo = (
            df_part.groupby(group_cols)[target_col]
                  .transform(lambda s: s.quantile(q_low) if (q_low is not None and len(s) >= min_samples) else np.nan)
            if q_low is not None else pd.Series(np.nan, index=df_part.index)
        )
        lo_g = float(np.nanquantile(y, q_low)) if q_low is not None else None
        hi_g = float(np.nanquantile(y, q_high)) if q_high is not None else None
        if q_high is not None: caps_hi = caps_hi.fillna(hi_g)
        if q_low  is not None: caps_lo = caps_lo.fillna(lo_g)

        yy = y.to_numpy()
        if clip_lower and q_low is not None and lo_g is not None and np.isfinite(lo_g):
            yy = np.maximum(yy, caps_lo.to_numpy())
        if clip_upper and q_high is not None and hi_g is not None and np.isfinite(hi_g):
            yy = np.minimum(yy, caps_hi.to_numpy())
        return pd.Series(yy, index=series.index)

    def _winsor_full_global(df_full, series, group_cols, q_low, q_high, clip_lower, clip_upper, min_samples):
        return _winsor_train_part(df_full, series, group_cols, q_low, q_high, clip_lower, clip_upper, min_samples)

    def _safe_iter(avg, default_min=100):
        if avg is None or (isinstance(avg, float) and np.isnan(avg)): return default_min
        try: return int(max(default_min, round(avg)))
        except Exception: return default_min

    # -------------------- 피처/타깃 준비 --------------------
    feat_ok = [c for c in feature_cols if (c in train_df.columns) and pd.api.types.is_numeric_dtype(train_df[c])]
    if not feat_ok:
        raise ValueError("유효한 feature가 없습니다.")
    log_cols = auto_select_log_cols(train_df, feat_ok)

    y_all_raw = np.clip(pd.to_numeric(train_df[target_col], errors='coerce').fillna(0).astype(float), 0, None)
    ...
    def _run_cv_once(xgb_params, lgb_params, sample_frac_for_metric=1.0, collect_best_iters=False):
        ...
        for tr_idx, val_idx in tscv.split(X_all):
            ...
            y_tr_raw = y_all_raw.iloc[tr_idx]
            y_val_raw = y_all_raw.iloc[val_idx]

            if use_winsor:
                y_tr_cap = _winsor_train_part(
                    train_df.iloc[tr_idx], y_tr_raw,
                    group_cols=win_group_cols, q_low=win_q_low, q_high=win_q_high,
                    clip_lower=win_clip_lower, clip_upper=win_clip_upper, min_samples=win_min_samples
                )
            else:
                y_tr_cap = y_tr_raw

            y_tr = np.log1p(y_tr_cap.to_numpy())
            y_val = np.log1p(y_val_raw.to_numpy())

            # --- XGB (중복 n_estimators 제거; fit에서 early_stopping 지정)
            xgb = XGBRegressor(
                **xgb_params,
                random_state=seed, n_jobs=-1, tree_method="hist", eval_metric="mae",
            ...
            oof_xgb[val_idx] = np.clip(np.expm1(px), 0, None)
            oof_lgb[val_idx] = np.clip(np.expm1(pl), 0, None)
        ...
        if sample_frac_for_metric < 1.0 and len(idx) > 200:
            ...
            sm = float(smape(y_all_raw.to_numpy()[sub], (w_xgb * oof_xgb + w_lgb * oof_lgb)[sub]))
        else:
            sm = float(smape(y_all_raw.to_numpy()[mask], pred_mix))
        return sm, (w_xgb * oof_xgb + w_lgb * oof_lgb), was_val, iters_x, iters_l

    # -------------------- Optuna: 하이퍼 탐색 --------------------
    best_bundle = {"xgb": None, "lgb": None, "smape": np.inf, "iters_x": None, "iters_l": None}
    ...
    def objective(trial):
        ...
        sm, _, _, itx, itl = _run_cv_once(
            xgb_params, lgb_params,
            sample_frac_for_metric=max(0.1, min(1.0, float(sample_frac))),
            collect_best_iters=True
        )
        if sm < best_bundle["smape"]:
            best_bundle.update({"smape": sm, "xgb": xgb_params, "lgb": lgb_params,
                                "iters_x": [i for i in itx if i is not None],
                                "iters_l": [i for i in itl if i is not None]})
        return sm
    ...
    try:
        study.optimize(objective, n_trials=n_trials, callbacks=[_tqdm_cb])
    finally:
        try:
            pbar.set_postfix_str(f"best_smape: {study.best_value:.6f}")
        except Exception:
    ...
    if not any(t.state == TrialState.COMPLETE for t in study.trials):
        raise RuntimeError("Optuna: 완료된 trial이 없습니다(첫 trial 이전 예외 가능).")

    # -------------------- 베스트 하이퍼로 OOF 전체 재계산 --------------------
    best_xgb = best_bundle["xgb"]; best_lgb = best_bundle["lgb"]
    ...
    if use_winsor:
        y_full_cap = _winsor_full_global(
            train_df, y_all_raw,
            group_cols=win_group_cols, q_low=win_q_low, q_high=win_q_high,
            clip_lower=win_clip_lower, clip_upper=win_clip_upper, min_samples=win_min_samples
        )
        y_full_log = np.log1p(y_full_cap.to_numpy())
    else:
    ...
    xgb_final = XGBRegressor(**{**best_xgb, "n_estimators": xgb_final_n, "random_state": seed, "n_jobs": -1, "tree_method": "hist", "eval_metric": "mae"})
    xgb_final.fit(X_full, y_full_log, verbose=False)

    lgb_final = LGBMRegressor(**{**best_lgb, "n_estimators": lgb_final_n, "random_state": seed, "n_jobs": -1})
...
pred1, oof1, oof1_s = model_segmented(
    ...
    n_splits=N_SPLITS, n_trials=N_TRIALS, sample_size=SAMPLE_SIZE
)
...
pred4, oof4, oof4_s = model_by_building_num(
    ...
    n_splits=N_SPLITS, n_trials=N_TRIALS, sample_size=SAMPLE_SIZE_BNUM
)
...
with open(save_path + "(LOG)model_{version}.txt", "a") as f:
    f.write(f"{version}\n")
    f.write(f"<SEED :{SEED}>\n")
    f.write(f"{filename}\n")
    f.write(f"N_SPLITS {N_SPLITS} | N_TRIALS {N_TRIALS} | AUGMENTATION AUGMENTATION {1+(60/AUGMENTATION)} times\n")
    f.write(f"SAMPLE_SIZE {SAMPLE_SIZE} | SAMPLE_SIZE_BNUM {SAMPLE_SIZE_BNUM}\n")
