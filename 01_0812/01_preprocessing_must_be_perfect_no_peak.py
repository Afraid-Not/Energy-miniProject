...
def train_ensemble_models_enhanced(X_train, y_train, X_val, y_val, is_insolation=False, use_log_scale=False, optimize_params=False):
    """Transition 모델이 추가된 강화된 앙상블 모델 (Peak 모델 제거, 5개 모델)"""
    ...
    print("    > 5개 모델 앙상블 훈련... (하락 + Transition 가중치 적용)")
    ...
    return {
        'models': (xgb_model, lgbm_model, decline_model, transition_model, cat_model),
        'final_mae': final_mae,
...
def predict_with_ensemble_enhanced(models_dict, X_test, is_insolation=False):
    """Transition 모델이 포함된 5개 모델 앙상블 예측 (Peak 모델 제거)"""
    xgb_model, lgbm_model, decline_model, transition_model, cat_model = models_dict['models']
    ...
    for i in range(len(ensemble_pred)):
        ...
        if is_transition:  # 전환 구간 (최우선)
            ensemble_pred[i] = (xgb_pred[i] * 0.15 + lgbm_pred[i] * 0.2 + 
                               decline_pred[i] * 0.15 + transition_pred[i] * 0.35 + 
                               cat_pred[i] * 0.15)
        else:  # 일반 구간
            ensemble_pred[i] = (xgb_pred[i] * 0.25 + lgbm_pred[i] * 0.25 + 
                               decline_pred[i] * 0.2 + transition_pred[i] * 0.15 + 
                               cat_pred[i] * 0.15)
...
# 결과 저장
train_all.to_csv(trainer + f'06_train_{SEED}_optuna_no_peak.csv', index=False)
test_all.to_csv(trainer + f'06_test_{SEED}_optuna_no_peak.csv', index=False)
...
with open(csv_path + "(LOG)01_preprocessing_optuna.txt", "a") as f:
    f.write(f"<SEED : {SEED}> no peak model\n")
...
print(f"[06_preprocessing] 종료 (시계열 연속성 + Optuna 최적화 + 하락 폭 보존 최적화 완료)")