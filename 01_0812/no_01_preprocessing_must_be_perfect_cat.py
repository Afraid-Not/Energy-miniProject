...
def optimize_hyperparameters(X_train_sample, y_train_sample, X_val_sample, y_val_sample, 
    ...
    # tqdm으로 진행 상황 표시
    with tqdm(total=n_trials, desc=f"{model_type.upper()} 최적화", leave=False) as pbar:
        def callback(study, trial):
            pbar.set_postfix({'Best RMSE': f'{study.best_value:.6f}'})
            pbar.update(1)
        ...
        if col in test_result.columns:
            for lag in lag_hours:
                ...
                for building_num in test_result['건물번호'].unique():
                    ...
                    # lag 시간만큼 train 마지막 데이터로 초기값 채우기
...
def train_ensemble_models_enhanced(X_train, y_train, X_val, y_val, is_insolation=False, use_log_scale=False, optimize_params=False):
    """Peak + Transition 모델이 추가된 강화된 앙상블 모델 (Optuna 최적화 포함, CatBoost 제거)"""
    ...
    print("    > 5개 모델 앙상블 훈련... (하락 + Peak + Transition 가중치 적용)")
    ...
    return {
        'models': (xgb_model, lgbm_model, decline_model, peak_model, transition_model),
        'final_mae': final_mae,
...
def predict_with_ensemble_enhanced(models_dict, X_test, is_insolation=False):
    """Peak + Transition 모델이 포함된 5개 모델 앙상블 예측 (CatBoost 제거)"""
    xgb_model, lgbm_model, decline_model, peak_model, transition_model = models_dict['models']
    ...
    for i in range(len(ensemble_pred)):
        ...
        if is_transition:  # 전환 구간 (최우선)
            ensemble_pred[i] = (xgb_pred[i] * 0.15 + lgbm_pred[i] * 0.2 + 
                               decline_pred[i] * 0.15 + peak_pred[i] * 0.15 + 
                               transition_pred[i] * 0.35)
        elif is_peak:  # Peak 구간
            ensemble_pred[i] = (xgb_pred[i] * 0.25 + lgbm_pred[i] * 0.25 + 
                               decline_pred[i] * 0.1 + peak_pred[i] * 0.3 + 
                               transition_pred[i] * 0.1)
        else:  # 일반 구간
            ensemble_pred[i] = (xgb_pred[i] * 0.25 + lgbm_pred[i] * 0.25 + 
                               decline_pred[i] * 0.2 + peak_pred[i] * 0.15 + 
                               transition_pred[i] * 0.15)
...
# 결과 저장
train_all.to_csv(trainer + f'06_train_{SEED}_no_catboost.csv', index=False)
test_all.to_csv(trainer + f'06_test_{SEED}_no_catboost.csv', index=False)

print(f"[8] 저장 완료 (시계열 연속성 반영 + Optuna 최적화, CatBoost 제거)")

# 결과 요약
print("\n" + "="*70)
print("최종 결과 요약 (시계열 연속성 + 전체 학습 + Optuna 최적화 앙상블, CatBoost 제거)")
print("="*70)
...
# 로그 파일에 결과 저장
with open(csv_path + "(LOG)01_preprocessing_no_catboost.txt", "a") as f:
    f.write(f"<SEED : {SEED}> (CatBoost 제거 + tqdm)\n")
...
print(f"[06_preprocessing] 종료 (시계열 연속성 + Optuna 최적화 + CatBoost 제거 + tqdm 진행표시 완료)")