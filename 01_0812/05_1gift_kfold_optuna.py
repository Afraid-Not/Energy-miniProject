...
def optimize_hyperparameters(x_sample, y_sample, n_trials=50):
    def objective(trial):
        ...
        # CatBoost 파라미터
        cat_params = {
            'n_estimators': trial.suggest_int('cat_n_estimators', 300, 1000),
            'learning_rate': trial.suggest_float('cat_learning_rate', 0.01, 0.2),
            'max_depth': trial.suggest_int('cat_max_depth', 3, 8),
        }
...
for bno in tqdm(building_ids, desc="Building Progress"):
    print(f"\n    > 🏢 건물번호 {bno} 모델링 중...")
    ...
    best_params = optimize_hyperparameters(x_sample, y_sample, n_trials=30)
    ...
    final_preds.extend(final_prediction)
...
filename = f"{SEED}_energy_KFold_Optuna_SMAPE_{score_str}.csv"
...
with open("./Energy/01/(LOG)Gift_model.txt", "a") as f:
    f.write(f"<{SEED} 회차>\n")
    f.write(f"<05_1gift_kfold_optuna.py>\n")
