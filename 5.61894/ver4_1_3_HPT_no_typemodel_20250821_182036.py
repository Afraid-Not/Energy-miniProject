...
def feature_engineering(df):
    ...
    # day_of_year
    df['temp_date']   = pd.to_datetime(df['일시'].str[:8], format='%Y%m%d', errors='coerce')
    df['day_of_year'] = df['temp_date'].dt.dayofyear.astype('Int64')
...
#        'SIN_day_of_year', 'COS_day_of_year', 'PT', 'CDH', 'DI', 'PVC_per_CA',
#        'ESS_installation', 'PCS_installation', 'Facility_Density',
#        'SIN_summer', 'COS_summer', 'groupID', 'sunrise_hour', 'sunset_hour',
#        'daylight_prev', 'daylight_instant', 'daylight_next', 'solar_elevation',
#        'holidays', 'dow_hour_mean', 'dow_hour_std', 'holiday_mean',
...
#        'day_of_year', 'SIN_day_of_year', 'COS_day_of_year', 'PT', 'CDH', 'DI',
#        'PVC_per_CA', 'ESS_installation', 'PCS_installation',
#        'Facility_Density', 'SIN_summer', 'COS_summer', 'groupID',
#        'sunrise_hour', 'sunset_hour', 'daylight_prev', 'daylight_instant',
#        'daylight_next', 'solar_elevation', 'holidays', 'dow_hour_mean',
#        'dow_hour_std', 'holiday_mean', 'holiday_std', 'hour_mean', 'hour_std',
#        'building_mean', 'building_std', 'sunshine', 'solar',
#        'power_consumption', 'is_interpolated'],
#       dtype='object')
# exit()
#endregion
...
def impute_solar_for_zero_bnos_cv_optuna(
    ...
    - Optuna: 학습 데이터의 30% 샘플로 n_trials회 튜닝(KFold 5-fold OOF SMAPE 최소화)
...
print(f"  [SEED_TRAIN_SOLAR 50]")

train_filled, smape_score = impute_solar_for_zero_bnos_cv_optuna(
    train=train,
    zero_bnos=zero_bnos,
    drop_cols=drop_cols,
    seed=50, reuse_optuna=True,
    n_splits=N_SPLITS,
    n_trials=N_TRIALS,
    sample_frac=SAMPLE_SIZE,
    cache_key = "impute-solar-solar-feat41e792cc"
)
...
test['sunshine'], oof_sunshine = train_predict_test_target_cv_optuna(
    ...
    reuse_optuna=True,
    cache_key = "predict-sunshine-sunshine-feat06a02b8e",
)

print("    > SMAPE :", oof_sunshine)

print(f"  [SEED_SOLAR 701]")
...
test['solar'], oof_solar = train_predict_test_target_cv_optuna(
    ...
    seed=701, reuse_optuna=True,
    n_splits=N_SPLITS, 
    n_trials=N_TRIALS, 
    sample_frac=SAMPLE_SIZE,
    restrict_daylight=True,
    cache_key = "predict-solar-solar-feat41e792cc"
)
...
def _train_one_model_cv_optuna(   # ← 이름 유지(호환)
    ...
    """
    TSS 교차검증 + Optuna로 XGB/LGB 하이퍼파라미터 튜닝 (SMAPE 최소화).
    ※ 윈저라이징은 사용하지 않음(파라미터는 호환을 위해 남겨둠).
    - 라벨은 원본을 log1p로 변환하여 학습, 예측은 expm1 복원.
    - 진행바는 오른쪽에 best_smape만 표시.
    - 최종 재학습: 베스트 하이퍼 + fold 평균 best_iteration.
    """
    ...
    # ---------- Optuna: 하이퍼 탐색 ----------
    best_bundle = {"xgb": None, "lgb": None, "smape": np.inf,
                   "iters_x": None, "iters_l": None}
    ...
    def objective(trial):
        ...
        if sm < best_bundle["smape"]:
            best_bundle.update({
                "smape": sm, "xgb": xgb_params, "lgb": lgb_params,
    ...
    # ---------- 베스트 하이퍼로 OOF 전체 재계산 ----------
    best_xgb = best_bundle["xgb"]
    best_lgb = best_bundle["lgb"]
    ...
    # 전체 라벨: 윈저링 없이 원본 → log1p
    y_full_log = np.log1p(y_all_raw.to_numpy())

    # fold 평균 best_iter로 고정
...
final_oof_smape = smape(
    train_clean.loc[mask, target].to_numpy(),
    oof_ens.loc[mask].to_numpy()
)
print(f"[OOF] Final Ensemble (M1+M4) SMAPE: {final_oof_smape}\n")
