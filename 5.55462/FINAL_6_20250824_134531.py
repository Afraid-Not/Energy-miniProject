...
def feature_engineering(df):
    ...
    df['Time'] = (
        ...
        pd.to_numeric(df['minute'], errors='coerce') / 60.0
    ).astype('float32')
    ...
    df['rainy']=0
    df.loc[(df['precipitation']>=0.1), 'rainy'] = 1
    
    yr = df['date'].dt.year.astype(str)
    season_start = pd.to_datetime(yr + "-06-01")
    season_end   = pd.to_datetime(yr + "-10-01")
...
def peak_holidays(df, is_train=True):
    ...
    if is_train :
        ...
        # df.loc[(df['building_num']==38) & (df['date'].dt.strftime('%m-%d').isin(['06-07'])), 'holidays'] = 2
        ...
        # df.loc[(df['building_num']==56) & (df['date'].dt.strftime('%m-%d').isin(['06-07','08-16'])), 'holidays'] = 2
...
numeric_cols= [
    ...
    'solar', 'sunshine', 'power_consumption'
]
...
drop_cols = [
    ...
    'holiday_mean', 'holiday_std', 'hour_mean', 'hour_std', 'building_mean',
    'building_std', 'power_consumption', 'building_num', 'precipitation'
]
...
drop_cols_for_sunshine = [
    ...
    'holidays', 'dow_hour_mean', 'dow_hour_std',
    'holiday_mean', 'holiday_std', 'hour_mean', 'hour_std', 'building_mean',
    'building_std', 'solar','power_consumption', 'building_num', 'precipitation'
]
...
pvc_features = [
    ...
    'temperature','rainy','windspeed','humidity',
    'hour','dow','day','day_of_year','minute', 'Time',
...
def model_by_type(train_clean, test, target='power_consumption', type_col='building_type', seed=SEED, n_splits=5, n_trials=20, sample_size=0.3):
    ...
    ess_for_type.remove('building_type')
    type_scores = {}
    oof_series_full = pd.Series(np.nan, index=train_clean.index)
...
def _subset_recent(df, n_splits, sample_size=None):
    """
    TimeSeriesSplit 최소길이 보장하면서 tail 기준으로 최근 데이터만 선택.
    sample_size: None | float(0~1) | int(개수)
    """
    min_rows = max(30, n_splits * 2)
    if sample_size is None:
        return df.tail(max(len(df), min_rows))
    if isinstance(sample_size, float) and 0 < sample_size < 1:
        k = int(len(df) * sample_size)
        return df.tail(max(k, min_rows))
    if isinstance(sample_size, int) and sample_size > 0:
        return df.tail(max(sample_size, min_rows))
    return df.tail(max(len(df), min_rows))

def model_by_building_num_pure(
    train_clean: pd.DataFrame,
    test: pd.DataFrame,
    target: str = 'power_consumption',
    bno_col: str = 'building_num',
    feature_list=None,
    seed: int = 73,
    n_splits: int = 5,
    n_trials: int = 30,
    sample_size=None,            # tail 샘플링 (비율/개수), None이면 전체
    sample_frac: float = 0.3,    # Optuna 내부 메트릭 산정시 행 서브샘플(속도↑)
    w_xgb: float = 0.5,
    w_lgb: float = 0.5,
    reuse_optuna: bool = True,
    cache_dir: str = "./Energy/03",
    cache_prefix: str = "m5pure"
):
    """
    PVC/ESS 구분 없이 feature_list만 사용해서
    건물번호별로 _train_one_model_cv_optuna로 튜닝-학습하고,
    _predict_with_models로 테스트 예측을 수행.
    반환: (pred_series_for_test, oof_smape_by_building(dict), oof_series_full)
    """
    feature_list = feature_list or pure_features

    # 출력 그릇
    pred = pd.Series(0.0, index=test.index, dtype='float64')
    oof_series_full = pd.Series(np.nan, index=train_clean.index, dtype='float64')
    b_scores = {}

    target_buildings = sorted(test[bno_col].dropna().unique())
    print(f"  [model_by_building_num_pure] activate (per-building, {len(target_buildings)} buildings)")

    for b in target_buildings:
        tr_b = train_clean[train_clean[bno_col] == b]
        te_b = test[test[bno_col] == b]

        if len(tr_b) < max(30, n_splits*2):
            print(f"    [Building {b}] skip (train rows < {max(30, n_splits*2)})")
            continue

        # 실제 존재하는 피처만 사용 (타이포/결측 컬럼 자동 제외)
        feats = [c for c in feature_list if c in tr_b.columns]
        if not len(feats):
            print(f"    [Building {b}] skip (no usable features)")
            continue

        # 최근 구간 샘플링(시계열 성질 유지)
        tr_use = _subset_recent(tr_b, n_splits=n_splits, sample_size=sample_size)

        # --- 학습(Optuna 튜닝 + OOF 산출 + 최종 모델 생성) ---
        try:
            xgb_final, lgb_final, med_full, feat_ok, log_cols, oof_smape, oof_series = _train_one_model_cv_optuna(
                train_df=tr_use, target_col=target, feature_cols=feats, seed=seed,
                n_splits=n_splits, n_trials=n_trials, sample_frac=sample_frac,
                w_xgb=w_xgb, w_lgb=w_lgb,
                reuse_optuna=reuse_optuna, cache_dir=cache_dir,
                cache_key=f"{cache_prefix}-b{int(b)}", cache_prefix=cache_prefix
            )
        except ValueError as e:
            # (예: 모든 usable feature가 비수치일 때 등) 안전 스킵
            print(f"    [Building {b}] skip ({e})")
            continue

        # --- OOF 기록 (원래 train_clean 인덱스에 반영) ---
        oof_series_full.loc[oof_series.index] = oof_series.values
        b_scores[int(b)] = float(oof_smape)
        print(f"    [Building {b}] OOF SMAPE: {oof_smape:.6f} (rows={len(tr_use)})")

        # --- 테스트 예측 ---
        models_pack = (xgb_final, lgb_final, med_full, feat_ok, log_cols)
        te_pred = _predict_with_models(te_b, models_pack, clip0=True)
        pred.loc[te_b.index] = te_pred.astype('float64')

    return pred, b_scores, oof_series_full
...
print("[M5] By BuildingNum (PURE FEATURES, OPTUNA)")
pred5, oof5, oof5_s = model_by_building_num_pure(
    train_clean, test, target=target, bno_col='building_num',
    feature_list=pure_features, seed=SEED,
    n_splits=N_SPLITS_MODEL, n_trials=N_TRIALS_MODEL,
    sample_size=SAMPLE_SIZE_BNUM,    # 있으면 사용
    sample_frac=SAMPLE_SIZE_MODEL,   # Optuna 내부 메트릭 서브샘플 비율(예: 0.3)
    w_xgb=0.5, w_lgb=0.5,
    reuse_optuna=True, cache_dir="./Energy/03", cache_prefix="m5pure"
)
avg5 = (sum(oof5.values()) / len(oof5)) if len(oof5) else float('nan')
print(f"[By BuildingNum PURE Model] SMAPE : {avg5}\n")
...
# ===== 앙상블 가중 (M1/M2/M4/M5 사용) =====
# W1, W2, W4, W5 = 0.25, 0.25, 0.25, 0.25
# pred_final = W1*pred1 + W2*pred2 + W4*pred4 + W5*pred5
W4, W5 = 0.5, 0.5
pred_final = W4*pred4 + W5*pred5

# ===== 최종 앙상블 OOF SMAPE =====
oof_df = pd.DataFrame({
    # 'm1': oof1_s,   # 세그먼트 OOF
    # 'm2': oof2_s,   # 유형별 OOF
    'm4': oof4_s,   # 번호별 OOO-aware OOF
    'm5': oof5_s,   # 번호별 PURE-feature OOF
})

# w = np.array([W1, W2, W4, W5], dtype=float)
w = np.array([W4, W5], dtype=float)
...
with open(save_path + f"(LOG)model_{version}.txt", "a") as f:
    ...
    f.write(f"[By BuildingNum PURE Model] SMAPE : {avg5}\n")
