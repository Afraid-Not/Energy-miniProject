...
train_call = '13_01_train_SEED65.csv'
test_call = '13_01_test_SEED65.csv'
...
peak_weight_manual=0.1

def train_and_predict_subset(train_subset, test_subset, weight=peak_weight_manual, name="Subset"):
    ...
    if building_type_for_peak:
        # peak_model은 이제 서브셋에 대한 예측값을 반환합니다.
        ...
        if peak_oof_pred_subset is not None and peak_test_pred_subset is not None:
            ...
            # 가중치 설정 (예시: 기본 모델 0.8, 피크 모델 0.2)
            weight_base = (1 - weight)
            weight_peak = (weight)

            # 가중치 앙상블 적용: 기본 모델 예측 + 피크 모델 예측
            oof_pred_final_combined[train_peak_idx_bool.values] = (
                weight_base * oof_pred_final_combined[train_peak_idx_bool.values] + 
                weight_peak * peak_oof_pred_subset
            )
            test_pred_final_combined[test_peak_idx_bool.values] = (
                weight_base * test_pred_final_combined[test_peak_idx_bool.values] + 
                weight_peak * peak_test_pred_subset
            )
...
with open("./Energy/13_submission/(LOG)13_02_model.txt", "a") as f:
    f.write(f"<파일명 : 13_02_model_peak_weight_no_retrain.py>\n")
    f.write(f"<SEED :{SEED}>\n")
    f.write(f"사용된 파일:\n")
    f.write(f"{train_call} / {test_call}\n")
    f.write(f"저장된 파일:\n")
    f.write(f"{filename}\n")
    f.write(f"peak_weight: {peak_weight_manual}\n")
