...
from sklearn.model_selection import TimeSeriesSplit, KFold # TimeSeriesSplit 추가
...
# TimeSeriesSplit 설정
# n_splits 값은 데이터의 크기와 검증 폴드의 수에 따라 조절할 수 있습니다.
# 여기서는 5개의 폴드로 나누도록 설정했습니다. (train_size + test_size가 전체 데이터를 커버)
tscv = TimeSeriesSplit(n_splits=5) 
kfol = KFold(n_splits=5, random_state=SEED, shuffle=True)
...
for bno in building_ids:
    ...
    # TimeSeriesSplit은 데이터를 시간 순서대로 분할하여, 과거 데이터로 학습하고 미래 데이터를 예측하는 시나리오를 모방합니다.
    # 여기서는 마지막 폴드를 최종 훈련/검증 세트로 사용했습니다.
    # 모든 폴드를 순회하며 모델을 훈련하고 예측을 앙상블할 수도 있지만, 코드의 복잡성을 줄이기 위해 최종 폴드만 사용했습니다.
    for train_index, val_index in kfol.split(x):
...
avg_smape_exp = np.expm1(avg_smape) # 이 값은 SMAPE 점수가 아니며, 로그 역변환된 평균값입니다.
# 실제 SMAPE는 이미 val_smape 변수에 계산되어 있으므로, avg_smape_exp는 삭제하거나 이름을 변경하는 것이 좋습니다.
...
print(f"[6] 📁 저장 완료 ")
print(f"✅ 최종 SMAPE 점수 : {avg_smape:.6f}") # avg_smape_exp 대신 avg_smape 사용
