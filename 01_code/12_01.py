...
building_smape_dict = {}  # 건물별 SMAPE 기록
building_pred_cache = {}  # 건물별 예측값 캐시
# ========================
# 건물별 모델 학습 및 예측
# ========================
...
for building_id in building_ids:
    ...
    if X_sunshine.empty or y_sunshine.empty:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_sunshine, y_sunshine)):
            ...
            sunshine_smape = mean_absolute_error(y_val, sun_val) # SMAPE로 변경
            print(f"    > Fold {fold +1} MAE : {sunshine_smape:.6f}")
    ...
    if X_insolation.empty or y_insolation.empty:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_insolation, y_insolation)):
            ...
            solar_smape = mean_absolute_error(y_val, solar_val) # SMAPE로 변경
            print(f"    > Fold {fold +1} MAE : {solar_smape:.6f}")
    ...
    print(f"  [BUILDING {building_id}] 잔차 보정 모델 학습 시작")

    # 잔차 계산: 실제값 - 기본 예측값 (훈련 데이터에 대해서만)
    base_oof_preds = (xgb_model.predict(X_train_final) + lgbm_model.predict(X_train_final)) / 2
    residual_target = y_train_final - base_oof_preds
    ...
    residual_model.fit(X_train_final, residual_target,
                       eval_set=[(X_train_final, residual_target)],
                       verbose=False)

    # 훈련 데이터에서 잔차 예측값 계산 (평가용)
    residual_pred_train = residual_model.predict(X_train_final)
    residual_mae = smape(residual_target, residual_pred_train)
    print(f"  [BUILDING {building_id}] 잔차 SMAPE: {residual_mae:.6f}")
    
    # test 데이터에 대한 잔차 예측 후 보정
    residual_pred = residual_model.predict(X_test_final)
    final_preds_building += residual_pred

    
    # 음수 예측 보정
    final_preds_building[final_preds_building < 0] = 0

    print(f"  [BUILDING {building_id}] 잔차 보정 완료")
    ...
    if fold_power_smapes:
        ...
        building_smape_dict[building_id] = avg_building_smape
        building_pred_cache[building_id] = final_preds_building.copy()  # 예측 결과 임시 저장
...
print(f"   > 기준 평균 SMAPE: {overall_avg_power_smape:.6f}")

for building_id, smape_val in building_smape_dict.items():
    if smape_val >= overall_avg_power_smape:
        print(f"\n🔁 [BUILDING {building_id}] 재학습 시작 (SMAPE {smape_val:.6f} ≥ 평균)")

        # 동일한 방식으로 재학습 (여기선 간단히 xgb만 사용 예시)
        train_building = train_all[train_all['건물번호'] == building_id].copy()
        test_building = test_all[test_all['건물번호'] == building_id].copy()
        ...
        xgb_model_retry = XGBRegressor(random_state=SEED, n_estimators=2000, learning_rate=0.02,
                                       subsample=0.8, colsample_bytree=0.8, max_depth=9,
                                       early_stopping_rounds=100, n_jobs=-1)
        xgb_model_retry.fit(X_train_final, y_train_final,
                            eval_set=[(X_train_final, y_train_final)],
                            verbose=False)
        
        retry_preds = xgb_model_retry.predict(X_test_final)
        retry_preds[retry_preds < 0] = 0

        # 대체 여부 확인
        prev_preds = building_pred_cache[building_id]
        prev_smape = smape(y_train_final, prev_preds[:len(y_train_final)])
        retry_smape = smape(y_train_final, xgb_model_retry.predict(X_train_final))

        print(f"    - 기존 SMAPE: {prev_smape:.6f}, 재학습 SMAPE: {retry_smape:.6f}")

        if retry_smape < prev_smape:
            print(f"    ✅ 개선됨 → 예측 결과 덮어씀")
            submission_df.loc[submission_df['건물번호'] == building_id, 'answer'] = retry_preds
        else:
            print(f"    ❌ 개선 안됨 → 기존 결과 유지")
