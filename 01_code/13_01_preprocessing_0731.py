...
""" # (파생피쳐 만들기 / Train 이상치 Nearest Neighbor k=5)
...
train_all_fixed = impute_train_solar_for_missing_bnos(train_all, zero_bnos, k=7)
test_filename_pre = f'pre_test.csv'

# print(f"[3] 전처리된 데이터 저장 중...")
train_filename = f'preprocessed_train.csv'
train_all_fixed.to_csv(save_path  + train_filename, index=False)
test_all.to_csv(save_path + test_filename_pre, index=False)
###########################################################################
 """

test_all = pd.read_csv(save_path + 'pre_test.csv')
train_all_fixed = pd.read_csv(save_path + 'preprocessed_train.csv')
...
for building_id in building_ids:
    ...
    train_feature = ['건물번호', '기온(°C)', '강수량(mm)', '풍속(m/s)', '습도(%)','연면적(m2)', '냉방면적(m2)', '태양광용량(kW)',
                     ...
                     '호텔', '불쾌지수', '태양광per냉방면적', 'ESS설치여부', 'PCS설치여부', '설비밀도', 'peak_time'] # Added peak_time
    ...
    if X_sunshine.empty or y_sunshine.empty:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_sunshine, y_sunshine)):
            ...
            # ==========================
            # Peak Time Model (11시 ~ 16시)
            # ==========================
            X_train_peak = X_train[X_train['peak_time'] == 1]
            y_train_peak = y_train[X_train['peak_time'] == 1]
            X_val_peak = X_val[X_val['peak_time'] == 1]
            y_val_peak = y_val[X_val['peak_time'] == 1]
            test1_peak_indices = test1[test1['peak_time'] == 1].index

            if not X_train_peak.empty:
                peak_model = LGBMRegressor(
                    random_state=SEED, n_estimators=500, learning_rate=0.05,
                    max_depth=6, num_leaves=48, reg_alpha=0.7, reg_lambda=1.5,
                    verbosity=-1
                )
                peak_model.fit(X_train_peak, y_train_peak,
                               eval_set=[(X_val_peak, y_val_peak)],
                               callbacks=[early_stopping(50, verbose=False), log_evaluation(0)])
                
                test1 = test1.reset_index(drop=True)  # ← 가장 중요!
                test1_peak_indices = test1[test1['peak_time'] == 1].index.to_numpy()
                # Apply peak model predictions only to peak hours in test set
                peak_test_preds = np.zeros(test1.shape[0])
                if test1_peak_indices.size > 0:
                    peak_test_preds[test1_peak_indices] = peak_model.predict(test1.loc[test1_peak_indices])
                    
                # Weighted average for final prediction
                # More weight to peak model during peak hours
                peak_weight = 0.5 # Adjust as needed
                final_test_combined = final_test.copy()
                final_test_combined[test1_peak_indices] = (1 - peak_weight) * final_test[test1_peak_indices] + peak_weight * peak_test_preds[test1_peak_indices]
                final_test = final_test_combined
            
            fold_mae = mean_absolute_error(y_val, final_val) # Still evaluate on original validation set for consistency
        ...
        print(f"      MAE: {avg_sunshine_mae_building:.6f}")
    ...
    if X_insolation.empty or y_insolation.empty:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_insolation, y_insolation)):
            ...
            # 최종 예측: 앙상블 + 잔차 보정
            final_pred_test = pred_test_ensemble + residual_pred_test
            final_pred_val = pred_val_ensemble + residual_model.predict(X_val)
            ...
            if not X_train_peak.empty:
                ...
                # More weight to peak model during peak hours
                peak_weight = 0.5 # Adjust as needed
                final_pred_test_combined = final_pred_test.copy()
                final_pred_test_combined[test1_peak_indices] = (1 - peak_weight) * final_pred_test[test1_peak_indices] + peak_weight * peak_test_preds[test1_peak_indices]
                final_pred_test = final_pred_test_combined

            fold_mae = mean_absolute_error(y_val, final_pred_val) # Evaluate on original validation set for consistency
            print(f"        > 최종 MAE : {fold_mae:.6f}")

            insolation_total_mae += fold_mae
            insolation_preds_building += final_pred_test / n_split

        avg_insolation_mae_building = insolation_total_mae / n_split
        test_building['일사(MJ/m2)'] = insolation_preds_building
        print(f"      MAE: {avg_insolation_mae_building:.6f}")
...
test_filename = f'13_01_test_SEED{SEED}.csv'


# Save preprocessed dataframes
test_all.to_csv(save_path +test_filename, index=False)
