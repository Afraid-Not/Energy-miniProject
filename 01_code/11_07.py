...
for btype in building_types:
    ...
    print(f"  >> '{btype}' 평균 SMAPE: {np.mean(smape_folds):.4f}, 선택된 fold: {best_fold+1}\n")
###############
print(f"    >> 완료")
...
for bno in building_ids:
    ...
    final_preds_by_building.extend(test_pred_folds)
...
print("[3] 전체 데이터 예측")
train_sorted = train.sort_values(['건물번호', '일시']).reset_index(drop=True)
test_sorted = test.sort_values(['건물번호', '일시']).reset_index(drop=True)

x_all = train_sorted[features]
y_all = np.log1p(train_sorted[target])
x_test_all = test_sorted[features]

val_preds_all_model = np.zeros(len(x_all))
test_preds_all_model = np.zeros(len(x_test_all))
val_smapes_all = []

kf_all = KFold(n_splits=N_SPLIT, shuffle=True, random_state=SEED)

for fold, (tr_idx, val_idx) in enumerate(kf_all.split(x_all)):
    x_train, x_val = x_all.iloc[tr_idx], x_all.iloc[val_idx]
    y_train, y_val = y_all.iloc[tr_idx], y_all.iloc[val_idx]

    # 🔥 건물유형별로 분리
    train_fold = train_sorted.iloc[tr_idx].copy()
    val_fold = train_sorted.iloc[val_idx].copy()

    # 유형별 예측 stack
    oof_train_preds, oof_val_preds, oof_test_preds = [], [], []

    for btype in train['건물유형'].unique():
        model_pack = pretrained_models[btype]

        # 해당 유형만 필터링
        x_tr_b = x_train[train_fold['건물유형'] == btype]
        y_tr_b = y_train[train_fold['건물유형'] == btype]
        x_val_b = x_val[val_fold['건물유형'] == btype]
        y_val_b = y_val[val_fold['건물유형'] == btype]
        x_test_b = x_test_all[test_sorted['건물유형'] == btype]

        if len(x_tr_b) == 0: continue  # skip 없는 유형

        scaler = clone(model_pack['scaler'])
        x_tr_scaled = scaler.fit_transform(x_tr_b)
        x_val_scaled = scaler.transform(x_val_b)
        x_test_scaled = scaler.transform(x_test_b)

        xgb = clone(model_pack['xgb'])
        lgb = clone(model_pack['lgb'])
        cat = clone(model_pack['cat'])

        xgb.fit(x_tr_scaled, y_tr_b, eval_set=[(x_val_scaled, y_val_b)], verbose=False)
        lgb.fit(x_tr_scaled, y_tr_b, eval_set=[(x_val_scaled, y_val_b)],
                callbacks=[early_stopping(50, verbose=False), log_evaluation(0)])
        cat.fit(x_tr_scaled, y_tr_b, eval_set=(x_val_scaled, y_val_b), early_stopping_rounds=50)

        # stacking
        pred_train = np.vstack([xgb.predict(x_tr_scaled), lgb.predict(x_tr_scaled), cat.predict(x_tr_scaled)]).T
        pred_val = np.vstack([xgb.predict(x_val_scaled), lgb.predict(x_val_scaled), cat.predict(x_val_scaled)]).T
        pred_test = np.vstack([xgb.predict(x_test_scaled), lgb.predict(x_test_scaled), cat.predict(x_test_scaled)]).T

        oof_train_preds.append((pred_train, train_fold['건물유형'] == btype))
        oof_val_preds.append((pred_val, val_fold['건물유형'] == btype))
        oof_test_preds.append((pred_test, test_sorted['건물유형'] == btype))

    # 전체 레벨2 학습
    oof_train_stack = np.zeros((len(x_train), 3))
    oof_val_stack = np.zeros((len(x_val), 3))
    oof_test_stack = np.zeros((len(x_test_all), 3))

    for pred, mask in oof_train_preds:
        oof_train_stack[mask.values] = pred
    for pred, mask in oof_val_preds:
        oof_val_stack[mask.values] = pred
    for pred, mask in oof_test_preds:
        oof_test_stack[mask.values] = pred

    # Meta + Final
    meta = RidgeCV()
    meta.fit(oof_train_stack, y_train)
    val_lvl2 = meta.predict(oof_val_stack)
    test_lvl2 = meta.predict(oof_test_stack)

    final_model = GradientBoostingRegressor(n_estimators=300, learning_rate=0.05, max_depth=3, random_state=SEED)
    final_model.fit(val_lvl2.reshape(-1, 1), y_val)

    val_final = final_model.predict(val_lvl2.reshape(-1, 1))
    smape_fold = np.mean(200 * np.abs(np.expm1(val_final) - np.expm1(y_val)) / (
        np.abs(np.expm1(val_final)) + np.abs(np.expm1(y_val)) + 1e-6))
    val_smapes_all.append(smape_fold)

    val_preds_all_model[val_idx] = val_final
    test_preds_all_model += final_model.predict(test_lvl2.reshape(-1, 1)) / N_SPLIT

print(f"  >> 전체 데이터 SMAPE (유형별 사전학습 모델): {np.mean(val_smapes_all):.6f}")
all_smape=np.mean(val_smapes_all)
print(f"  >> SMAPE : {all_smape:.6f}")

# 정렬
samplesub_sorted = samplesub.sort_values(['건물번호', '일시']).reset_index(drop=True)

# 두 모델 예측값 앙상블
final_preds_by_building = np.array(final_preds_by_building)
final_preds_by_all = test_preds_all_model
ensemble_preds = (final_preds_by_building + final_preds_by_all) / 2

samplesub_sorted['answer'] = ensemble_preds
samplesub = samplesub_sorted.sort_index()

# 평가
ensemble_smape = (np.mean(val_smapes) + np.mean(val_smapes_all)) / 2
print(f"  >> 최종 앙상블 SMAPE : {ensemble_smape:.6f}")

today = datetime.datetime.now().strftime('%Y%m%d')
score_str = f"{ensemble_smape:.4f}".replace('.', '_')
filename = f"11_06_ENS_bno_all_{today}_SMAPE_{score_str}.csv"
samplesub.to_csv(save_path + filename, index=False)

with open("./Energy/11_submission/11_06_log.txt", "a") as f:
    f.write(f"<SEED :{SEED}>\n")
    f.write(f"{filename}\n")
    f.write(f"건물별 SMAPE : {np.mean(val_smapes)}\n")
    f.write(f"전체모델 SMAPE : {np.mean(val_smapes_all)}\n")
    f.write(f"앙상블 SMAPE : {ensemble_smape}\n")
    f.write("="*40 + "\n")
