...
def predict_train_solar_with_all_features(train_df, zero_bnos, kfold=5):
    ...
    def train_models_with_all_features(df_filtered, label='일사(MJ/m2)', kfold=kfold):
        ...
        for fold, (train_idx, valid_idx) in enumerate(kf.split(X)):
            ...
            score = mean_absolute_error(y_valid_with_hour['y_true'].values, pred_mean)
            scores.append(score)
...
test_filename = f'13_01_test_SEED{SEED}_up.csv'
train_filename = f'13_01_train_SEED{SEED}_up.csv'
...
with open(save_path + "(LOG)13_01_preprocessing.txt", "a") as f:
    f.write(f"<파일명 : 13_01_preprocessing_train_upgrade_rolling_features_no_peak_train_predict.py>\n")
    f.write(f"<SEED : {SEED}>\n")
