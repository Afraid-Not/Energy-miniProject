# ========================
# 임포트 및 랜덤 시드 고정
print(f"[12_02_preprocessing] 시작")
# ========================
import pandas as pd
import numpy as np
import os
...
seed_file = "./Energy/12_submission/12_02_pre.json"
...
""" (파생피쳐 만들기 / Train 이상치 Nearest Neighbor 7)
...
test_filename_un = f'un_preprocessed_{SEED}_test.csv'

# print(f"[3] 전처리된 데이터 저장 중...")
train_filename = f'preprocessed_{SEED}_train.csv'
...
train_all_fixed = pd.read_csv(save_path + 'train_csv/preprocessed_15_train.csv')
...
for building_id in building_ids:
    ...
    if X_insolation.empty or y_insolation.empty:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_insolation, y_insolation)):
            ...
            fold_mae = mean_absolute_error(y_val, final_pred_val)
            print(f"      > 최종 MAE : {fold_mae:.6f}")
...
test_filename = f'preprocessed_{SEED}_test.csv'
...
with open(save_path + "12_02_preprocessing.txt", "a") as f:
    f.write(f"<SEED : {SEED}>\n")
...
print(f"[12_02_preprocessing] 종료")