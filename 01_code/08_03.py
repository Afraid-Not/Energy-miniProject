...
# print(train_e.shape)      #(181560, 34)
# print(test_e.shape)      #(22440, 34)
# print(train_e.columns)
# Index(['건물번호', '일시', '기온(°C)', '강수량(mm)', '풍속(m/s)', '습도(%)', '일조(hr)',
#        '일사(MJ/m2)', '전력소비량(kWh)', '건물유형', '연면적(m2)', '냉방면적(m2)', '태양광용량(kW)',
#        'ESS저장용량(kWh)', 'PCS용량(kW)', 'has_solar', 'has_ess', 'has_pcs', '시간',
#        '요일', '주말여부', '기온×습도', '풍속×기온', '강수여부', '근무시간', 'sin_hour', 'cos_hour',
...
# train.to_csv('./Energy/moremore.csv', index=False)
# exit()
# ----------------------------
# 일조/일사 예측 보조모델
# ----------------------------
...
#        'has_solar', 'has_ess', 'has_pcs', '시간', '요일', '주말여부', '강수여부', '근무시간',
#        'sin_hour', 'cos_hour', '건물유형_LE', '요일_0', '요일_1', '요일_2', '요일_3',
#        '요일_4', '요일_5', '요일_6', '건물유형_IDC(전화국)', '건물유형_건물기타', '건물유형_공공',
#        '건물유형_백화점', '건물유형_병원', '건물유형_상용', '건물유형_아파트', '건물유형_연구소', '건물유형_학교',
#        '건물유형_호텔'],
# exit()
...
sun_feature_cols = ['건물번호', '기온(°C)', '강수량(mm)', '풍속(m/s)', '습도(%)', 'sin_hour', 'cos_hour', 'DI']
train_cols = ['기온(°C)', '강수량(mm)', '풍속(m/s)', '습도(%)', 'sin_hour', 'cos_hour', 'DI']

sun_test_preds = []
sun_rmses = []
# feature_importances_sun = []
...
# feature_importances_df = pd.concat(feature_importances_sun, axis=0)

# # 건물별 평균 중요도
# avg_importance = feature_importances_df.groupby(['Feature', 'Target'])['Importance'].mean().reset_index()
# import matplotlib.pyplot as plt
# import seaborn as sns
# # 시각화
# plt.figure(figsize=(12, 6))
# sns.barplot(data=avg_importance, x='Importance', y='Feature', hue='Target')
# plt.title('평균 Feature Importance (일조, 일사 예측)')
# plt.tight_layout()
# plt.show()
# exit()
# 불필요한 컬럼 제거
