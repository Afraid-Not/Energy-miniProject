...
peak_weigh_manual=0.1
def predict_train_solar_with_peak_blending(train_df, zero_bnos, kfold=5, peak_weight=peak_weigh_manual):
    print(f"\n[일사 보간] 일반 + 피크 모델 블렌딩 시작 (peak_weight={peak_weight})")
    ...
    # 전체용 모델
    model_all= train_models(train_df.copy(), label='일사(MJ/m2)')

    # 피크 전용 모델
    train_peak = train_df.copy()
    train_peak['hour'] = pd.to_datetime(train_peak['일시']).dt.hour
    train_peak = train_peak[(train_peak['hour'] >= 10) & (train_peak['hour'] <= 17)]
    model_peak= train_models(train_peak, label='일사(MJ/m2)')
    ...
    for bno in zero_bnos:
        ...
        pred_all = np.mean([model.predict(X_target) for model in model_all], axis=0)
        pred_peak = np.mean([model.predict(X_target) for model in model_peak], axis=0)

        is_peak = target.loc[target_idx, 'hour'].between(10, 16)
        blended_pred = np.where(is_peak, (1 - peak_weight) * pred_all + peak_weight * pred_peak, pred_all)

        df_filled.loc[target_idx, '일사(MJ/m2)'] = np.clip(blended_pred, 0, None)
...
train_all_fixed = predict_train_solar_with_peak_blending(train_all, zero_bnos, kfold=5)

# print(f"[전체 시간대] 평균 PWMAE: {score_prepro_all:.6f}")
# print(f"[Peak 시간대] 평균 PWMAE: {score_prepro_peak:.6f}")
...
with open(save_path + "(LOG)13_01_preprocessing.txt", "a") as f:
    f.write(f"<파일명 : 13_01_preprocessing_peak_model_train_predict_bleding.py>\n")
    f.write(f"<SEED : {SEED}>\n")
    f.write(f"{train_filename}\n")
    f.write(f"{test_filename}\n")
    f.write(f"Peak_WEIGHT : {peak_weigh_manual}\n")
