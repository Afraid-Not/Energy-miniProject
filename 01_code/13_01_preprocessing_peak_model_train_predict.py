...
def predict_train_solar_for_missing_bnos(train_df, zero_bnos, kfold=5, peak_only=False):
    mode = "PEAK 시간대 전용" if peak_only else "전체 시간대"
    print(f"\n      > train 일사(MJ/m2) 모델 기반 보간 시작 ({mode})")

    df_filled = train_df.copy()
    df_filled['hour'] = pd.to_datetime(df_filled['일시']).dt.hour

    if peak_only:
        df_filled = df_filled[(df_filled['hour'] >= 11) & (df_filled['hour'] <= 16)]

    # 훈련용 데이터
    train_data = df_filled[
        (~df_filled['건물번호'].isin(zero_bnos)) & 
        (df_filled['일사(MJ/m2)'] > 0)
    ].copy()

    X = train_data[['기온(°C)', '습도(%)', '풍속(m/s)', 'hour']].values
    y = train_data['일사(MJ/m2)'].values
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(X)):
        ...
        score = peak_weighted_mae(y_valid, y_pred)
        pwmae_scores.append(score)
        print(f"         > Fold {fold+1} PWMAE: {score:.6f}")

    train_pwmae_scores = np.mean(pwmae_scores)
    print(f"      > 평균 peak_weighted_mae: {train_pwmae_scores:.6f}\n")

    # ======================
    # 전체 학습 후 zero_bnos 예측
    # ======================
    print("      > 전체 데이터로 모델 재학습 및 보간 진행...")
    ...
    # 보간 대상 전체 사용 (peak_only=False 시 전체 시간 포함)
    df_target_full = train_df.copy()
    df_target_full['hour'] = pd.to_datetime(df_target_full['일시']).dt.hour

    for bno in zero_bnos:
        print(f"      > [BUILDING {bno}] 예측 중...")
        target_rows = df_target_full[(df_target_full['건물번호'] == bno) & (df_target_full['일사(MJ/m2)'] == 0)].copy()

        if target_rows.empty:
            continue

        target_rows['hour'] = pd.to_datetime(target_rows['일시']).dt.hour
        target_rows.loc[(target_rows['hour'] < 5) | (target_rows['hour'] > 21), '일사(MJ/m2)'] = 0.0

        # 예측 대상은 peak_only 설정에 따라 달라짐
        condition = (target_rows['hour'] >= 5) & (target_rows['hour'] <= 21)
        if peak_only:
            condition &= (target_rows['hour'] >= 10) & (target_rows['hour'] <= 17)

        valid_idx = target_rows[condition].index
        X_target = target_rows.loc[valid_idx, ['기온(°C)', '습도(%)', '풍속(m/s)', 'hour']].values

        pred_lgb = model_lgb.predict(X_target)
        pred_xgb = model_xgb.predict(X_target)
        pred_cat = model_cat.predict(X_target)

        preds_mean = np.mean([pred_lgb, pred_xgb, pred_cat], axis=0)
        df_target_full.loc[valid_idx, '일사(MJ/m2)'] = np.clip(preds_mean, 0, None)

    return df_target_full.drop(columns='hour'), train_pwmae_scores
...
train_all_fixed, score_all = predict_train_solar_for_missing_bnos(train_all, zero_bnos, kfold=5, peak_only=False)
# PEAK 시간대 예측
train_peak_fixed, score_peak = predict_train_solar_for_missing_bnos(train_all, zero_bnos, kfold=5, peak_only=True)

print(f"[전체 시간대] 평균 PWMAE: {score_all:.6f}")
print(f"[Peak 시간대] 평균 PWMAE: {score_peak:.6f}")
    ...
    if X_sunshine.empty or y_sunshine.empty:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_sunshine, y_sunshine)):
            ...
            if not X_train_peak.empty and len(X_train_peak) > 1: # 최소 2개 이상의 샘플 필요
                ...
                final_val = np.where(peak_hours_val_mask, ((1-peak_weight)* final_val_base) + (peak_weight* pred_peak_val), final_val_base)
                final_test = np.where(peak_hours_test_mask, ((1-peak_weight)*final_test_base) + (peak_weight*pred_peak_test), final_test_base)
            else:
...
train_filename = f'13_01_train_SEED{SEED}.csv'
...
with open(save_path + "(LOG)13_01_preprocessing.txt", "a") as f:
    ...
    f.write(f"[전체 시간대] 평균 PWMAE: {score_all:.6f}\n")
    f.write(f"[Peak 시간대] 평균 PWMAE: {score_peak:.6f}\n")    
