...
seed_file = "./Energy/03/log/(prototype)SEED_COUNTS.json"
...
py_path = './Energy/03/'
save_path = f'./Energy/03/{SEED}_(proto)submission/'
...
test['sunshine'], oof_sunshine = train_predict_test_target_cv_optuna(
    ...
    seed=SEED, n_splits=3, n_trials=20, sample_frac=0.5,
...
def _train_one_model_cv_optuna(
    ...
    """
    target: log1p 변환. 피처 중 일부는 auto log1p.
    Optuna(30% 샘플, 30회, 5fold OOF SMAPE). 최종 FULL 재학습 후 (xgb,lgb,med,log_cols,OOF_pred) 반환.
    """
    ...
    Xs = X_all.loc[sample_idx]
    ys = y_log.loc[sample_idx]
    ...
    study = _optimize_with_pbar(objective, n_trials=n_trials, desc="optuna(power)", seed=seed)
    ...
    oof_smape,
    pd.Series(oof_pred, index=train_df.index)  # ← OOF 예측(원스케일)
)
...
with open(log_path + "(LOG)model_prototype.txt", "a") as f:
    f.write(f"<SEED :{SEED}>\n")
    f.write(f"{filename}\n")
    f.write(f"[Segmented] SMAPE {avg1:.6f}\n")
    f.write(f"[Type] SMAPE {avg2:.6f}\n")
