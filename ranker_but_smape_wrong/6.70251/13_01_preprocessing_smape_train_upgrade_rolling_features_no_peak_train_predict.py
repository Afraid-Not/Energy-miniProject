...
def predict_train_solar_with_all_features(train_df, zero_bnos, kfold=5):
    ...
    def train_models_with_all_features(df_filtered, label='일사(MJ/m2)', kfold=kfold):
        ...
        for fold, (train_idx, valid_idx) in enumerate(kf.split(X)):
            ...
            score = smape(y_valid_with_hour['y_true'].values, pred_mean)
            scores.append(score)
            print(f"      > Fold {fold+1}: {score:.6f} (SMAPE)")

        score_prepro = np.mean(scores)
        print(f"      > 평균 SMAPE: {score_prepro:.6f}")
...
for building_id in building_ids:
    ...
    if X_sunshine.empty or y_sunshine.empty:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_sunshine, y_sunshine)):
            ...
            fold_mae = smape(y_val, final_val)
            print(f"         > 최종 smape : {fold_mae:.6f}")
        ...
        print(f"      [일조(hr)] smape (건물별 평균): {avg_sunshine_mae_building:.6f}")
    ...
    if X_insolation.empty or y_insolation.empty:
        ...
        print(f"      [일사(MJ/m2)] smape (건물별 평균): {avg_insolation_mae_building:.6f}")
...
test_filename = f'13_01_test_SEED{SEED}_smape.csv'
train_filename = f'13_01_train_SEED{SEED}_smape.csv'
...
print(f"\n[5] 전체 평균 일조(hr) smape: {final_avg_sunshine_mae:.6f}")
print(f"[5] 전체 평균 일사(MJ/m2) smape: {final_avg_insolation_mae:.6f}")

with open(save_path + "(LOG)13_01_preprocessing.txt", "a") as f:
    f.write(f"<파일명 : 13_01_preprocessing_smape_train_upgrade_rolling_features_no_peak_train_predict.py>\n")
    f.write(f"<SEED : {SEED}>\n")
    f.write(f"{train_filename}\n")
    f.write(f"{test_filename}\n")
    f.write(f"Overall Average 일조(hr) smape : {final_avg_sunshine_mae:.6f}\n")
    f.write(f"Overall Average 일사(MJ/m2) smape : {final_avg_insolation_mae:.6f}\n")
    f.write("="*40 + "\n")
