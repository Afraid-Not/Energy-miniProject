...
print(f"    After Feature Engineering  - train : {train_filled.shape} | test : {test.shape}")  # (612000, 55) (16800, 55)
...
# Index([ '일시', 'temperature', 'precipitation', 'windspeed',
#        'humidity', 'sunshine', 'solar', 'power_consumption', 'building_type',
#        'all_area', 'cooling_area', 'pvc', 'ess', 'pcs', 'date', 'hour', 'dow',
#        'month', 'day', 'SIN_hour', 'COS_hour', 'SIN_day', 'COS_day',
#        'SIN_month', 'COS_month', 'SIN_dow', 'COS_dow', 'day_of_year',
...
pvc_features_for_segmented = [
    ...
    'pvc','pv_day_kwh_est','pv_hour_kwh_est','pvc_per_day',
    'solar_by_pvc','pv_to_ess_kwh_cap','ess_hours_at_pvc',
    'PVC_per_CA','Facility_Density','building_num'
]

no_pvc_features_for_segmented = [
    'building_type','groupID','holidays','peak',
    'temperature','precipitation','windspeed','humidity',
    'hour','dow','day','day_of_year',
    'SIN_hour','COS_hour','SIN_day','COS_day','SIN_month','COS_month',
    ...
    'hour_mean','hour_std','building_mean','building_std',
    'solar','solar_day_kwh_m2','sunshine_day_hours',
    'all_area','cooling_area', 'building_num'
]
...
def model_segmented(train_clean, test, target='power_consumption', seed=SEED):
    ...
    if len(tr_pvc) > 50 and seg_pvc_te.any():
        ...
        xgb,lgb,med,feats,logs,oof, oof_series = _train_one_model_cv_optuna(
            tr_pvc, target, _valid_features(train_clean, test, pvc_features_for_segmented),
    ...
    if len(tr_none) > 50 and seg_none_te.any():
        ...
        xgb,lgb,med,feats,logs,oof, oof_series = _train_one_model_cv_optuna(
            tr_none, target, _valid_features(train_clean, test, no_pvc_features_for_segmented),
            seed=seed, n_splits=3, n_trials=1, sample_frac=0.05   # ← 호환용 인자(무시 가능)
        )
...
def model_by_building_num(train_clean, test, target='power_consumption',
    ...
    for b in sorted(test[bno_col].dropna().unique()):
        ...
        # --- 피처 선택
        feat_list = pvc_features_for_num if has_pvc else no_pvc_features_for_num
        feat_list = list(feat_list)  # 혹시 튜플/Index일 수 있으니 리스트화
        drop_feats = {'building_type', 'groupID'}  # 여기에 더 넣어도 됨

        # 잘못된 remove 대신 리스트 컴프리헨션으로 필터링
        feat_list = [f for f in feat_list if f not in drop_feats]
