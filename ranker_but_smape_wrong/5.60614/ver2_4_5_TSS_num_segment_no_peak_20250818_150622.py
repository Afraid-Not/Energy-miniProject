...
import datetime
save_path = f'./Energy/03/{SEED}_submission_ver2_4_5/'
...
#        'groupID', 'holidays', 'dow_hour_mean', 'dow_hour_std',
...
#        'SIN_summer', 'COS_summer', 'groupID', 'holidays',
...
def model_segmented(train_clean, test, target='power_consumption', seed=SEED):
    print("  [model_segmented] activate (PVC vs No-PVC)")
    ...
    if len(tr_pvc) > 50 and seg_pvc_te.any():
        ...
        xgb,lgb,med,feats,logs,oof, oof_series = _train_one_model_cv_optuna(
            ...
            seed=seed, n_splits=3, n_trials=1, sample_frac=0.05   # ← 호환용 인자(무시 가능)
        )
...
# -------------------- 건물번호별 모델 (PVC 존재 여부에 따라 피처 자동 선택) --------------------
def model_by_building_num(train_clean, test, target='power_consumption',
                          bno_col='building_num', pvc_col='pvc', seed=SEED):
    print("  [model_by_building_num] activate (per-building, pvc-aware)")
    ...
    for b in sorted(test[bno_col].dropna().unique()):
        ...
        # --- 건물 단위 PVC 존재 여부 판단(학습/테스트 중 하나라도 pvc>0 있으면 True)
        ...
        # --- 피처 선택
        feat_list = pvc_features if has_pvc else no_pvc_features
        feats = _valid_features(train_clean, test, feat_list)

        print(f"    [Building {b}] using {'pvc_features' if has_pvc else 'no_pvc_features'} "
              f"(features={len(feats)})")

        # --- 학습(TSS 기반 _train_one_model_cv_optuna) & 예측
...
# ==================== RUN: 건물번호별 + Segmented(PVC/No-PVC) → 단일 최종 결과 ====================
target = 'power_consumption'

print("[MN] By BuildingNum (PVC-aware features)")
pred_num, oof_map_num, oof_series_num = model_by_building_num(
    train_clean, test, target=target, bno_col='building_num', pvc_col='pvc', seed=SEED
)
avg_num = (sum(oof_map_num.values()) / len(oof_map_num)) if len(oof_map_num) else float('nan')
print(f"[By BuildingNum Model] SMAPE : {avg_num}\n")

print("[MS] Segmented (PVC/No-PVC)")
pred_seg, oof_map_seg, oof_series_seg = model_segmented(
    train_clean, test, target=target, seed=SEED
)
avg_seg = (sum(oof_map_seg.values()) / len(oof_map_seg)) if len(oof_map_seg) else float('nan')
print(f"[Segmented Model] SMAPE : {avg_seg}\n")

# ---- 최종 하나의 예측: (건물번호별 ⨉ WN) + (세그먼트 ⨉ WS)
WN, WS = 0.50, 0.50   # ← 필요시 조정
pred_final_series = (
    WN * pd.Series(pred_num, index=test.index) +
    WS * pd.Series(pred_seg, index=test.index)
)
submission['answer'] = pred_final_series.values

# ---- 최종 하나의 OOF SMAPE (row-wise 가중 앙상블)
oof_df = pd.DataFrame({
    'num': oof_series_num,  # 건물번호별 OOF
    'seg': oof_series_seg,  # 세그먼트 OOF
})
w = np.array([WN, WS], dtype=float)
W = pd.DataFrame(np.broadcast_to(w, oof_df.shape),
                 index=oof_df.index, columns=oof_df.columns)

# NaN 예측엔 가중치 0 → 행별 재정규화
...
final_oof_smape = smape(train_clean.loc[mask, target].to_numpy(),
                        oof_ens.loc[mask].to_numpy())

print(f"[OOF] Final (NUM x {WN:.2f} + SEG x {WS:.2f}) SMAPE: {final_oof_smape:.6f}\n")

# ---- 저장: “하나의” 결과 파일만
today = datetime.datetime.now().strftime('%Y%m%d')
score_str = ("nan" if np.isnan(final_oof_smape) else f"{final_oof_smape:.4f}").replace('.', '_')
wtag = f"num{int(WN*100):02d}_seg{int(WS*100):02d}"
filename = f"(Num+Seg){SEED}_{today}_{wtag}_SMAPE_{score_str}.csv"
submission.to_csv(save_path + filename, index=False)
print(f"[SAVE] {filename}")

# ---- 로그
with open(save_path + "(LOG)model_TSS.txt", "a") as f:
    f.write(f"<SEED :{SEED}> (TSS Model ONE: NUM+SEG)\n")
    f.write(f"{filename}\n")
    f.write(f"[By BuildingNum] SMAPE {avg_num:.6f}\n")
    f.write(f"[Segmented] SMAPE {avg_seg:.6f}\n")
    f.write(f"[Final (num {WN:.2f} + seg {WS:.2f})] SMAPE {final_oof_smape:.6f}\n")
    f.write("="*40 + "\n")
