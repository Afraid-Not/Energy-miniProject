...
def model_segmented(train_clean, test, target='power_consumption', seed=SEED):
    print("  [model_segmented] activate (ESS/PVC/NoPVC)")

    # --- 세그먼트 마스크
    pvc_tr = (train_clean.get('pvc', 0).fillna(0) > 0)
    pcs_tr = (train_clean.get('pcs', 0).fillna(0) > 0)
    segs_tr = {
        'ESS'  : pvc_tr & pcs_tr,
        'PVC'  : pvc_tr & (~pcs_tr),
        'NoPVC': ~pvc_tr
    ...
    pvc_te = (test.get('pvc', 0).fillna(0) > 0)
    pcs_te = (test.get('pcs', 0).fillna(0) > 0)
    segs_te = {
        'ESS'  : pvc_te & pcs_te,
        'PVC'  : pvc_te & (~pcs_te),
        'NoPVC': ~pvc_te
    ...
    # 세그별 피처셋 선택
    feat_map = {
        'ESS'  : (ess_features if 'ess_features' in globals() else pvc_features),
        'PVC'  : pvc_features,
        'NoPVC': no_pvc_features,
    }
    ...
    for name in ['ESS', 'PVC', 'NoPVC']:
        tr_mask = segs_tr[name]
        te_mask = segs_te[name]
        tr_seg = train_clean.loc[tr_mask]
        te_seg = test.loc[te_mask]
        if len(tr_seg) <= 50:
            print(f"    [{name}] skip (train rows <= 50): {len(tr_seg)}")
            continue

        feats = _valid_features(train_clean, test, feat_map[name])
        print(f"    [{name}] train={len(tr_seg)} test={te_mask.sum()} feats={len(feats)}")

        # === 학습 (OOF 생성은 테스트 존재 여부와 무관하게 수행)
        xgb, lgb, med, fs, logs, oof, oof_series = _train_one_model_cv_optuna(
            tr_seg, target, feats, seed=seed, n_splits=5, n_trials=1, sample_frac=1
        )

        # oof_series 인덱스 강제 정렬 (혹시 배열로 반환될 경우 대비)
        if not isinstance(oof_series, pd.Series):
            oof_series = pd.Series(oof_series, index=tr_seg.index, dtype=float)
        else:
            oof_series = oof_series.reindex(tr_seg.index)

        oof_series_full.loc[tr_seg.index] = oof_series
        seg_scores[name] = oof
        print(f"      > OOF SMAPE({name}) : {oof:.6f}")

        # === 테스트 예측(테스트에 해당 세그가 있을 때만)
        if te_mask.any():
            pred.loc[te_seg.index] = _predict_with_models(te_seg, (xgb, lgb, med, fs, logs))

    # 세그먼트별 성능 dict, 전체 OOF 시리즈 반환
    return pred.values, seg_scores, oof_series_full
...
def model_by_type(train_clean, test, target='power_consumption', type_col='building_type', seed=SEED):
    ...
    for t in sorted(test[type_col].dropna().unique()):
        ...
        xgb,lgb,med,fs,logs,oof, oof_series = _train_one_model_cv_optuna(tr_t, target, feats,
                                                             seed=seed, n_splits=5, n_trials=1, sample_frac=0.05)
