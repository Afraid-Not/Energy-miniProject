...
import datetime
save_path = f'./Energy/03/{SEED}_submission_ver2_4_2/'
...
# ==================== RUN: 건물번호별 + Global → 단일 최종 결과 ====================
target = 'power_consumption'

print("[M] By BuildingNum (PVC-aware features)")
...
print("[MG] Global")
pred_glb, oof_glb, oof_series_glb = model_global(train_clean, test, target=target, seed=SEED)
print(f"[Global Model] SMAPE : {oof_glb}\n")

# ---- 최종 하나의 예측: (건물번호별 ⨉ WN) + (글로벌 ⨉ WG)
WN, WG = 0.60, 0.40   # ← 필요시 조정
pred_final_series = (
    WN * pd.Series(pred_num, index=test.index) +
    WG * pd.Series(pred_glb, index=test.index)
)
...
oof_df = pd.DataFrame({
    ...
    'glb': oof_series_glb,  # 글로벌 OOF
})
w = np.array([WN, WG], dtype=float)
...
print(f"[OOF] Final (NUM x {WN:.2f} + GLB x {WG:.2f}) SMAPE: {final_oof_smape:.6f}\n")

# ---- 저장: “하나의” 결과 파일만
today = datetime.datetime.now().strftime('%Y%m%d')
score_str = ("nan" if np.isnan(final_oof_smape) else f"{final_oof_smape:.4f}").replace('.', '_')
wtag = f"num{int(WN*100):02d}_glb{int(WG*100):02d}"
filename = f"(num+glb_one){SEED}_{today}_{wtag}_SMAPE_{score_str}.csv"
submission.to_csv(save_path + filename, index=False)
print(f"[SAVE] {filename}")

# ---- 로그
with open(save_path + "(LOG)model_TSS.txt", "a") as f:
    f.write(f"<SEED :{SEED}> (TSS Model ONE)\n")
    f.write(f"{filename}\n")
    f.write(f"[By BuildingNum] SMAPE {avg_num:.6f}\n")
    f.write(f"[Global] SMAPE {oof_glb:.6f}\n")
    f.write(f"[Final (num {WN:.2f} + glb {WG:.2f})] SMAPE {final_oof_smape:.6f}\n")
    f.write("="*40 + "\n")
