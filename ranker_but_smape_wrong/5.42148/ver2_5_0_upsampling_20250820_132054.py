...
import datetime
save_path = f'./Energy/03/{SEED}_submission_ver2_5_0/'
...
def upsample_20min_linear(
    df: pd.DataFrame,
    time_col: str,                   # 예: '일시'
    group_cols=None,                 # 예: ['building_num'] 또는 ['건물번호']
    numeric_cols=None,               # 지정 없으면 수치형 전부
    copy_non_numeric_from='prev',    # 새 행의 비수치 컬럼: 'prev'면 이전값, 'next'면 다음값, None이면 NaN
    enforce_hour_gap=False           # True면 정확히 1시간 간격일 때만 보간
) -> pd.DataFrame:
    df = df.copy()
    ...
    def _one_group(g: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(n - 1):
            ...
            # 1시간 간격 강제 여부
            if enforce_hour_gap and (nxt[time_col] - cur[time_col]) != pd.Timedelta(hours=1):
                continue

            # 20분 간격으로 2개 선형 보간 (:20, :40)
            for k, frac in enumerate([1/4, 2/4, 3/4], start=1):
                new = cur.copy()
                new[time_col] = cur[time_col] + pd.Timedelta(minutes=20 * k)
...
def peak_holidays(df, is_train=True):
    ...
    if is_train :
        ...
        df.loc[(df['building_num']==29) & (df['date'].dt.strftime('%m-%d').isin(['06-10','06-23','07-10','07-28','08-10'])), 'holidays'] = 1
        ...
        df.loc[(df['building_num']==29) & (df['date'].dt.strftime('%m-%d').isin(['08-25'])), 'holidays'] = 1
