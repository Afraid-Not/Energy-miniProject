...
def train_ensemble_models_enhanced(X_train, y_train, X_val, y_val, is_insolation=False, use_log_scale=False):
    ...
    final_rmse = np.sqrt(mean_squared_error(y_val_reset, ensemble_pred_val))
...
# ============================================================================
# 전체 데이터 학습 함수들 (Train 일사량 보간용)
# ============================================================================

def train_global_model_with_building_features(train_data, features, target_col, building_col='건물번호', scaler_type='standard'):
    """
    전체 데이터로 학습하되 건물별 특성을 반영하는 모델 훈련 (Train 일사량 보간용)
    """
    print(f"전체 데이터로 {target_col} 모델 학습 시작... (건물번호 피처 포함, {scaler_type} 스케일링)")
...
def predict_with_global_model(test_data, global_models, label_encoder, 
    ...
    """
    전체 모델로 건물별 예측 수행 (Train 일사량 보간용)
    """
    predictions = {}
...
def train_all_building_models_with_retrain(train_data, features, target_col, scaler_type='standard', max_retrain_attempts=2):
    """
    모든 건물별 모델 훈련 + RMSE 기반 재학습 (Test 일조/일사량 예측용)
    """
    global xgb_params, lgbm_params  # global 선언을 함수 시작 부분으로 이동
    
    print(f"모든 건물별 {target_col} 모델 학습 시작... ({scaler_type} 스케일링, RMSE 기반 재학습)")
    ...
    # 1차 학습: 모든 건물별로 모델 훈련
    print(f"[1차 학습] 모든 건물별 초기 모델 훈련...")
    for building_num in tqdm(train_data['건물번호'].unique(), desc=f"건물별 {target_col} 1차 모델 훈련"):
        ...
        rmse_scores.append(building_model['final_rmse'])
    
    # 평균 RMSE 계산
    avg_rmse = np.mean(rmse_scores) if rmse_scores else 0
    avg_mae = np.mean(mae_scores) if mae_scores else 0
    
    print(f"    > 1차 학습 완료 - 평균 MAE: {avg_mae:.6f}, 평균 RMSE: {avg_rmse:.6f}")
    print(f"    > RMSE 기준선: {avg_rmse:.6f} (이상인 건물들 재학습 예정)")
    
    # 2차 학습: RMSE가 평균 이상인 건물들 재학습
    poor_performance_buildings = []
    for building_num, model_dict in building_models.items():
        if model_dict['final_rmse'] > avg_rmse and model_dict['model_type'] == 'ensemble':
            poor_performance_buildings.append(building_num)
    
    if poor_performance_buildings:
        print(f"[재학습 대상] {len(poor_performance_buildings)}개 건물: {poor_performance_buildings}")
        
        for attempt in range(max_retrain_attempts):
            print(f"[{attempt+2}차 학습] 성능 개선 재학습 시도...")
            retrained_buildings = []
            
            for building_num in poor_performance_buildings.copy():
                building_data = train_data[train_data['건물번호'] == building_num].copy()
                ...
                # 하이퍼파라미터 조정으로 재학습
                print(f"    > 건물 {building_num}번 재학습 중... (이전RMSE: {building_models[building_num]['final_rmse']:.6f})")
                
                # 더 강화된 파라미터로 재학습
                original_xgb_params = xgb_params.copy()
                original_lgbm_params = lgbm_params.copy()
                ...
                # 재학습 수행
                retrained_model = train_building_specific_model(
                    building_data, features, target_col, building_num, scaler_type
                )
                
                # 성능 개선 확인
                if retrained_model['final_rmse'] < building_models[building_num]['final_rmse']:
                    building_models[building_num] = retrained_model
                    retrained_buildings.append(building_num)
                    retrain_log.append(f"건물 {building_num}: RMSE {building_models[building_num]['final_rmse']:.6f} -> {retrained_model['final_rmse']:.6f}")
                    print(f"    > 건물 {building_num}번 성능 개선됨! RMSE: {retrained_model['final_rmse']:.6f}")
                else:
                    print(f"    > 건물 {building_num}번 성능 개선 실패, 기존 모델 유지")
                
                # 파라미터 복원
                xgb_params = original_xgb_params
                lgbm_params = original_lgbm_params
            
            # 재학습된 건물들을 다음 반복에서 제외
            for building_num in retrained_buildings:
                if building_num in poor_performance_buildings:
                    poor_performance_buildings.remove(building_num)
            
            if not poor_performance_buildings:
                print(f"    > 모든 건물의 성능이 개선되었습니다!")
                break
    
    # 최종 성적 계산
    final_mae_scores = [model['final_mae'] for model in building_models.values()]
    final_rmse_scores = [model['final_rmse'] for model in building_models.values()]
    final_avg_mae = np.mean(final_mae_scores) if final_mae_scores else 0
    final_avg_rmse = np.mean(final_rmse_scores) if final_rmse_scores else 0
    
    print(f"    > 최종 결과 - 평균 MAE: {final_avg_mae:.6f}, 평균 RMSE: {final_avg_rmse:.6f}")
    
    if retrain_log:
        print(f"    > 재학습 성공 로그:")
        for log in retrain_log:
            print(f"      {log}")
    
    return building_models, final_avg_mae, final_avg_rmse

def predict_with_all_building_models(test_data, building_models, features, target_col):
    """
    모든 건물별 모델로 예측 수행 (Test 일조/일사량 예측용)
    """
...
print(f"[1] 전처리 시작 (Train 전체학습 + Test 건물별학습 + RMSE 재학습)")
...
print(f"[2] train zero_bno 일사 예측 시작 (전체 데이터 학습)")
...
# train zero_bno 일사량 예측을 위한 전체 데이터 모델 훈련
print(f"[2.1] 일사량이 있는 전체 데이터로 모델 훈련...")
...
print(f"[3] train zero_bno 일사 예측 완료 - Validation MAE: {insolation_models['final_mae']:.6f}")

########################## test 일조시간 예측 ############################
print(f"[4] test 일조시간 예측 시작 (건물별 학습 + RMSE 재학습)")
...
sunshine_models, avg_sunshine_mae, avg_sunshine_rmse = train_all_building_models_with_retrain(
...
print(f"[5] test 일조시간 예측 완료 - 평균 MAE: {avg_sunshine_mae:.6f}, 평균 RMSE: {avg_sunshine_rmse:.6f}")

########################## test 일사량 예측 ############################
print(f"[6] test 일사량 예측 시작 (건물별 학습 + RMSE 재학습)")
...
insolation_test_models, avg_insolation_test_mae, avg_insolation_test_rmse = train_all_building_models_with_retrain(
...
print(f"[7] test 일사량 예측 완료 - 평균 MAE: {avg_insolation_test_mae:.6f}, 평균 RMSE: {avg_insolation_test_rmse:.6f}")
...
# 결과 저장
train_all.to_csv(csv_path + f'06_train_{SEED}_optimized.csv', index=False)
test_all.to_csv(csv_path + f'06_test_{SEED}_optimized.csv', index=False)

print(f"[8] 저장 완료 (최적화된 학습 전략)")

# 결과 요약
print("\n" + "="*60)
print("최종 결과 요약 (Train 전체학습 + Test 건물별학습 + RMSE 재학습)")
print("="*60)
print(f"Train 일사량 예측 (전체모델) MAE: {insolation_models['final_mae']:.6f}")
print(f"Test 일조시간 예측 (건물별모델) 평균 MAE: {avg_sunshine_mae:.6f}, 평균 RMSE: {avg_sunshine_rmse:.6f}")
print(f"Test 일사량 예측 (건물별모델) 평균 MAE: {avg_insolation_test_mae:.6f}, 평균 RMSE: {avg_insolation_test_rmse:.6f}")

# 건물별 성능 분석
print(f"\n건물별 일조시간 예측 성능 분석:")
sunshine_rmses = [model['final_rmse'] for model in sunshine_models.values() if model['model_type'] == 'ensemble']
if sunshine_rmses:
    print(f"  - 최고 성능: RMSE {min(sunshine_rmses):.6f}")
    print(f"  - 최저 성능: RMSE {max(sunshine_rmses):.6f}")
    print(f"  - 표준편차: {np.std(sunshine_rmses):.6f}")

print(f"\n건물별 일사량 예측 성능 분석:")
insolation_rmses = [model['final_rmse'] for model in insolation_test_models.values() if model['model_type'] == 'ensemble']
if insolation_rmses:
    print(f"  - 최고 성능: RMSE {min(insolation_rmses):.6f}")
    print(f"  - 최저 성능: RMSE {max(insolation_rmses):.6f}")
    print(f"  - 표준편차: {np.std(insolation_rmses):.6f}")

# 로그 파일에 결과 저장
with open(csv_path + "(LOG)06_preprocessing.txt", "a") as f:
    f.write(f"<SEED : {SEED}> - Train 전체학습 + Test 건물별학습 + RMSE 재학습\n")
    f.write(f"Train 일사량 예측 (전체모델) MAE: {insolation_models['final_mae']:.6f}\n")
    f.write(f"Test 일조(hr) (건물별모델) 평균 MAE: {avg_sunshine_mae:.6f}, 평균 RMSE: {avg_sunshine_rmse:.6f}\n")
    f.write(f"Test 일사(MJ/m2) (건물별모델) 평균 MAE: {avg_insolation_test_mae:.6f}, 평균 RMSE: {avg_insolation_test_rmse:.6f}\n")
    f.write("개선사항: Train 전체학습으로 일사 보간, Test 건물별 특화 학습, RMSE 재학습으로 성능 최적화\n")
    f.write("핵심 전략: 전체 vs 개별 학습 최적 조합, 자동 재학습 시스템, 시계열 연속성, 하락 패턴 보존\n")
    
    if sunshine_rmses:
        f.write(f"일조 성능분석: 최고{min(sunshine_rmses):.6f} 최저{max(sunshine_rmses):.6f} 표준편차{np.std(sunshine_rmses):.6f}\n")
    if insolation_rmses:
        f.write(f"일사 성능분석: 최고{min(insolation_rmses):.6f} 최저{max(insolation_rmses):.6f} 표준편차{np.std(insolation_rmses):.6f}\n")
    f.write("="*40 + "\n")
    
print(f"[06_preprocessing] 종료 (최적화된 학습 전략 + RMSE 재학습 완료)")