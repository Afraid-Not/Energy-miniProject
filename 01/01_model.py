...
seed_file = "./Energy/01/(SEED_COUNT)01_model.json"
...
test_call = '06_test_43_.csv'
train_call = '06_train_43_.csv'

train = pd.read_csv(csv_path + train_call)
...
# --- 4. Optuna 가중 평균 최적화 및 최종 예측 ---
print("\n--- [Step 4] Optuna 가중 평균 최적화 및 최종 예측 시작 ---")

def optimize_ensemble_weights(type_wise_oof, building_wise_oof, group_oof, y_true, n_trials=100):
    """
    Optuna를 사용하여 세 모델의 최적 가중치를 찾습니다.
    """
    def objective(trial):
        # 가중치 탐색 공간 정의 (합이 1이 되도록 제약)
        w1 = trial.suggest_float('w_type', 0.0, 1.0)
        w2 = trial.suggest_float('w_building', 0.0, 1.0)
        w3 = trial.suggest_float('w_group', 0.0, 1.0)
        
        # 가중치 정규화 (합이 1이 되도록)
        total_weight = w1 + w2 + w3
        if total_weight == 0:
            return float('inf')  # 모든 가중치가 0인 경우 방지
        ...
        # 가중 평균 계산
        weighted_pred = w1_norm * type_wise_oof + w2_norm * building_wise_oof + w3_norm * group_oof
        
        # SMAPE 계산 (최소화 목표)
        current_smape = smape(np.expm1(y_true), np.expm1(weighted_pred))
        return current_smape
    ...
    # 최적 가중치 정규화
    best_w1 = study.best_params['w_type']
    best_w2 = study.best_params['w_building']
    best_w3 = study.best_params['w_group']
    
    total_weight = best_w1 + best_w2 + best_w3
    best_w1_norm = best_w1 / total_weight
    best_w2_norm = best_w2 / total_weight
    best_w3_norm = best_w3 / total_weight
    
    print(f"    > Optuna 최적 가중치:")
    print(f"      - 유형별 모델: {best_w1_norm:.4f}")
    print(f"      - 건물별 모델: {best_w2_norm:.4f}")
    print(f"      - 그룹별 모델: {best_w3_norm:.4f}")
    print(f"    > Optuna 최적 SMAPE: {study.best_value:.6f}")
    
    return best_w1_norm, best_w2_norm, best_w3_norm, study.best_value

# 최적 가중치 탐색
y_true_log = np.log1p(train[target])
optimal_w1, optimal_w2, optimal_w3, best_ensemble_smape = optimize_ensemble_weights(
    type_wise_oof_preds, building_wise_oof_preds, group_oof_preds, y_true_log, n_trials=100
)

# 최적 가중치로 앙상블 예측 계산
ensemble_oof_preds = (optimal_w1 * type_wise_oof_preds + 
                      optimal_w2 * building_wise_oof_preds + 
                      optimal_w3 * group_oof_preds)

ensemble_test_preds = (optimal_w1 * type_wise_test_preds + 
                       optimal_w2 * building_wise_test_preds + 
                       optimal_w3 * group_test_preds)

print(f"--- 가중 평균 앙상블 SMAPE: {best_ensemble_smape:.6f} ---")
...
def optimize_residual_model(X_train_scaled, residuals_target, n_trials=50):
    """
    잔차 모델의 하이퍼파라미터를 Optuna로 최적화합니다.
    """
    def objective(trial):
        # LGBMRegressor 하이퍼파라미터 탐색 공간
        param = {
            'objective': 'mae',
        ...
        for train_idx, val_idx in kf.split(X_train_scaled):
            ...
            y_train_fold, y_val_fold = residuals_target.iloc[train_idx], residuals_target.iloc[val_idx]
    ...
    print(f"    > 잔차 모델 최적 MAE: {study.best_value:.6f}")
    print(f"    > 잔차 모델 최적 파라미터: {study.best_params}")
...
# 잔차 모델 하이퍼파라미터 최적화
best_residual_params = optimize_residual_model(X_train_scaled_res, pd.Series(residuals), n_trials=50)

# 최적 파라미터로 잔차 모델 학습
residual_model = LGBMRegressor(objective='mae', random_state=SEED, **best_residual_params, n_jobs=-1, verbose=-1)
residual_model.fit(X_train_scaled_res, residuals)

# 테스트 데이터에 대한 잔차 예측
predicted_residuals = residual_model.predict(X_test_scaled_res)

# 최종 예측: 가중 앙상블 예측 + 예측된 잔차
...
print(f"--- [Step 4] Optuna 가중 평균 최적화 및 최종 예측 완료 ---")
...
filename = f"01_{today}_SMAPE_{score_str}_OptWeighted.csv"
samplesub.to_csv(save_path + filename, index=False)

with open("./Energy/01/(LOG)01_model.txt", "a") as f:
    f.write(f"<SEED :{SEED}>\n")
    f.write(f"{filename}\n")
    f.write(f"{test_call}\n")
    ...
    f.write(f"Optuna 가중 앙상블 SMAPE : {best_ensemble_smape}\n")
    f.write(f"최종 잔차학습 SMAPE : {final_oof_smape}\n")
    f.write(f"최적 가중치 - 유형별: {optimal_w1:.4f}, 건물별: {optimal_w2:.4f}, 그룹별: {optimal_w3:.4f}\n")
    f.write("="*40 + "\n")
