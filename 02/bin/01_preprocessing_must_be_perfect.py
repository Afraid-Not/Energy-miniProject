...
def create_day_pattern_mapping(train_df):
    ...
    if '요일' not in train_df.columns:
        train_df['date'] = pd.to_datetime(train_df['일시'])
        train_df['요일'] = train_df['date'].dt.dayofweek
    
    # 요일별 전력소비량 평균 계산 (건물번호별)
    building_dayofweek_avg = train_df.groupby(['건물번호', '요일'])['전력소비량(kWh)'].mean().reset_index()
    building_dayofweek_avg.columns = ['건물번호', '요일', 'avg_power_by_dayofweek']
    ...
    return day_pattern_mapping[['건물번호', '요일', 'dayoff', 'hot_day']]
...
def add_dayoff_hotday_feature(df):
    ...
    """
    # 요일별 전력소비량 평균 계산 (건물번호별)
    building_dayofweek_avg = df.groupby(['건물번호', '요일'])['전력소비량(kWh)'].mean().reset_index()
    building_dayofweek_avg.columns = ['건물번호', '요일', 'avg_power_by_dayofweek']
    ...
    df_with_features = df.merge(
        day_pattern_info[['건물번호', '요일', 'is_dayoff', 'is_hot_day']], 
        on=['건물번호', '요일'], 
        how='left'
    )
...
def feature_engineering(df, is_train=True):
    ...
    building_type_ohe = pd.get_dummies(df['건물유형'], prefix='건물유형')
...
def remove_first_day_with_nan_features(df, hours_to_remove=6):
    """
    ...
    if len(lag_cols) > 0:
        ...
        for building_num in df['건물번호'].unique():
            ...
            remove_count = min(hours_to_remove // 6, len(building_data)) 
            
            if remove_count > 0:
                indices_to_remove = building_data.head(remove_count).index
                df_cleaned = df_cleaned.drop(indices_to_remove)
        
        print(f"    > 각 건물별 첫 {hours_to_remove}시간 데이터 제거: {len(df)} -> {len(df_cleaned)} 행")
        return df_cleaned
...
day_pattern_mapping = create_day_pattern_mapping(train)
...
test_all = apply_day_pattern_features(test_all, day_pattern_mapping)
...
# 예측에 사용할 피처 선택
insolation_features = ['기온(°C)', '강수량(mm)', '풍속(m/s)', '습도(%)',
    'peak_time', '이슬점온도', '이슬점차이', '체감온도', 'SIN_시', 'COS_시', 
    '시각', 'SIN_일', 'COS_일', 'SIN_요일', 'COS_요일', '대기투명도',
    'rainy', 'high_humidity', 'cloudy_or_rain', 'humid_x_rain',  
    'cloudy_based_on_humidity', 'sunrise_hour', 'sunset_hour', 'daylight', 
    'solar_elevation', 'solar_rel_pos', 'extraterrestrial_rad',
    '강수시작', '강수끝', '날씨불안정지수',
    'discomfort_index', 'THI', 'CDH',
    '일조(hr)'
]
# '기온_급변', '습도_급변', '풍속_급변', 
...
sunshine_features = ['기온(°C)', '강수량(mm)', '풍속(m/s)', '습도(%)',
    'peak_time', '이슬점온도', '이슬점차이', 'SIN_시', 'COS_시', 
    '시각', 'SIN_일', 'COS_일', 'SIN_요일', 'COS_요일', '대기투명도',
    'rainy', 'high_humidity', 'cloudy_or_rain', 'humid_x_rain',  
    'cloudy_based_on_humidity', 'sunrise_hour', 'sunset_hour', 'daylight', 
    'solar_elevation', 'solar_rel_pos', 'extraterrestrial_rad',
