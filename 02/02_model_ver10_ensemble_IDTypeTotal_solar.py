...
seed_file = "./Energy/02/log/(model_ver10)SEED_COUNTS.json"
...
# ---- 공통 헬퍼: date 보장 + 마지막 1주일 검증 분할 ----
def ensure_datetime_col(df: pd.DataFrame, time_col: str = "date", fallback_col: str = "일시") -> pd.DataFrame:
    if time_col not in df.columns:
        if fallback_col not in df.columns:
            raise KeyError(f"'{time_col}'도 '{fallback_col}'도 없습니다.")
        try:
            df = df.copy()
            df[time_col] = pd.to_datetime(df[fallback_col].astype(str), format="%Y%m%d %H", errors="raise")
        except Exception:
            df = df.copy()
            df[time_col] = pd.to_datetime(df[fallback_col], errors="coerce")
        if df[time_col].isna().any():
            raise ValueError(f"'{fallback_col}' 파싱 실패가 있습니다.")
    return df

def last_week_split(df: pd.DataFrame, time_col: str = "date", val_days: int = 7):
    """마지막 1주일을 검증으로 사용(랜덤 X). 부족하면 90:10 순차 분할로 폴백."""
    df = df.sort_values(time_col)
    if df.empty:
        return df, df
    cutoff = df[time_col].max() - pd.Timedelta(days=val_days)
    g_val = df[df[time_col] >= cutoff]
    g_tr  = df[df[time_col] <  cutoff]
    # 폴백: 전부 밸리데이션이 되거나(train 없음) 전부 트레인이 되면(검증 없음) 90:10 순차
    if g_val.empty or g_tr.empty:
        cut = max(1, int(len(df) * 0.9))
        g_tr, g_val = df.iloc[:cut], df.iloc[cut:]
    return g_tr, g_val
...
def date_feat(df) :
    ...
    df['일사x태양광'] = df['일사(MJ/m2)'] * df['태양광용량(kW)'] * 0.85
...
# === Hybrid+Global (no KFold) + Optuna with per-scope sampling & tqdm best-only progress ===
...
W_FINAL_TYPE, W_FINAL_BUILD, W_FINAL_GLOBAL = 0.3, 0.3, 0.4  # final 3-way ensemble

# HPO trial counts (원하면 여기서 조정)
N_TRIALS_TYPE   = 20
N_TRIALS_BNO    = 20
N_TRIALS_GLOBAL = 30

# Sampling sizes per scope
SAMPLING_TYPE   = 0.3
SAMPLING_BNO    = 0.6
SAMPLING_GLOBAL = 0.1
...
# 2) Buffers
test_pred_type   = np.zeros(len(test), dtype=float)   # Type LGB
test_pred_build  = np.zeros(len(test), dtype=float)   # BNO XGB+LGB
...
def _tqdm_optimize(objective, n_trials, desc):
    ...
    # 진행바 아래에 최종 베스트 SMAPE를 '기록'으로 남김
    pbar.write(f"[{desc}] best SMAPE: {study.best_value:.4f}")
    pbar.close()
    return study

# 3) Global backup (90/10 split) models for fallback & also used for global split score
def fit_global_lgb_split():
...
global_lgb_split, global_med_split, global_val_smape_split = fit_global_lgb_split()
global_xgb_split, _                 , global_val_smape_xgb   = fit_global_xgb_split()  # med 동일 구조이므로 global_med_split 사용

# ===== 4-A) Type-level (LGBM only, Optuna + sampling 0.3) =====
for t in test[TYPE_COL].unique():
    ...
    # --- sampling (0.3)
    tr_samp = tr_type.sample(frac=SAMPLING_TYPE, random_state=SEED) if len(tr_type) > 0 else tr_type
    if 'date' in tr_samp.columns and TIME_AWARE:
        tr_samp = tr_samp.sort_values('date')
        split = int(len(tr_samp) * 0.9)
        g_tr, g_val = tr_samp.iloc[:split], tr_samp.iloc[split:]
    else:
        g_tr, g_val = train_test_split(tr_samp, test_size=0.1, random_state=SEED, shuffle=True)

    X_tr, y_tr = g_tr[feature_cols], np.log1p(g_tr[TARGET])
    X_val, y_val = g_val[feature_cols], np.log1p(g_val[TARGET])
    med = X_tr.median()
    X_tr_f = X_tr.fillna(med); X_val_f = X_val.fillna(med)
    ...
    def obj_type(trial):
        params = {
            ...
            "num_leaves": trial.suggest_int("lgb_num_leaves", 64, 512),
            "max_depth": trial.suggest_categorical("lgb_max_depth", [-1, 6, 8, 10, 12]),
            "min_child_samples": trial.suggest_int("lgb_min_child_samples", 10, 120),
        ...
        val_log, _ = _predict_lgb(model, X_val_f)
        smape = SMAPE(np.expm1(y_val), np.clip(np.expm1(val_log), 0, None))
        trial.set_user_attr("lgb_best_iter", getattr(model, "best_iteration_", None))
        return smape

    study_t = _tqdm_optimize(obj_type, N_TRIALS_TYPE, desc=f"type {t}")
    best_params_t = study_t.best_params
    best_iter_t   = study_t.best_trial.user_attrs.get("lgb_best_iter", None)

    # --- refit on FULL group (no sampling) with best params
    X_full = tr_type[feature_cols]; y_full = np.log1p(tr_type[TARGET])
    med_full = X_full.median()
    X_full_f = X_full.fillna(med_full)
    n_est = int(best_iter_t) if (best_iter_t is not None and best_iter_t > 0) else 2000
    lgb_final = LGBMRegressor(
        n_estimators=n_est,
        learning_rate=best_params_t["lgb_learning_rate"],
        num_leaves=best_params_t["lgb_num_leaves"],
        max_depth=best_params_t["lgb_max_depth"],
        min_child_samples=best_params_t["lgb_min_child_samples"],
        subsample=best_params_t["lgb_subsample"],
        colsample_bytree=best_params_t["lgb_colsample_bytree"],
        reg_lambda=best_params_t["lgb_reg_lambda"],
        random_state=SEED, n_jobs=-1, verbosity=-1
    )
    lgb_final.fit(X_full_f, y_full)
    X_te = te_type[feature_cols].fillna(med_full)
    preds_log = lgb_final.predict(X_te)
    preds = np.clip(np.expm1(preds_log), 0, None)
    test_pred_type[te_type.index] = preds

    # holdout score 기록(참고)
    type_group_means.append(study_t.best_value)
    metrics_type.append({
        "group": str(t), "split": "optuna_holdout_sampled",
        "rows": len(tr_type), "val_rows": len(X_val), "val_smape": study_t.best_value, "used_global": 0
    })

# ===== 4-B) Building-number level (XGB+LGB, Optuna + sampling 0.6) =====
for bno in test[BNO_COL].unique():
    ...
    # --- sampling (0.6)
    tr_samp = tr_b.sample(frac=SAMPLING_BNO, random_state=SEED) if len(tr_b) > 0 else tr_b
    if 'date' in tr_samp.columns and TIME_AWARE:
        tr_samp = tr_samp.sort_values('date')
        split = int(len(tr_samp) * 0.9)
        g_tr, g_val = tr_samp.iloc[:split], tr_samp.iloc[split:]
    ...
    # --- Optuna joint objective for (XGB + LGB) ensemble
    def obj_bno(trial):
        # XGB params
        xgb_params = dict(
            n_estimators=2000,
            learning_rate=trial.suggest_float("xgb_eta", 0.01, 0.1, log=True),
        ...
        px_log, _, xgb_best = _predict_xgb(xgb, X_val_f)
        pl_log, lgb_best    = _predict_lgb(lgb, X_val_f)
        val_pred = W_XGB * np.clip(np.expm1(px_log), 0, None) + W_LGB * np.clip(np.expm1(pl_log), 0, None)
        smape = SMAPE(np.expm1(y_val), val_pred)

        trial.set_user_attr("xgb_best_iter", xgb_best)
        trial.set_user_attr("lgb_best_iter", lgb_best)
        return smape

    study_b = _tqdm_optimize(obj_bno, N_TRIALS_BNO, desc=f"bno {bno}")
    best_params_b = study_b.best_params
    xgb_best_it   = study_b.best_trial.user_attrs.get("xgb_best_iter", None)
    lgb_best_it   = study_b.best_trial.user_attrs.get("lgb_best_iter", None)

    # --- refit on FULL group with best params
    X_full = tr_b[feature_cols]; y_full = np.log1p(tr_b[TARGET])
    med_full = X_full.median()
    X_full_f = X_full.fillna(med_full)

    xgb_final = XGBRegressor(
        n_estimators=int(xgb_best_it) if (xgb_best_it is not None and xgb_best_it > 0) else 2000,
        learning_rate=best_params_b["xgb_eta"],
        max_depth=best_params_b["xgb_max_depth"],
        subsample=best_params_b["xgb_subsample"],
        colsample_bytree=best_params_b["xgb_colsample_bytree"],
        min_child_weight=best_params_b["xgb_min_child_weight"],
        reg_lambda=best_params_b["xgb_reg_lambda"],
        random_state=SEED, n_jobs=-1, tree_method="hist",
        eval_metric="mae"
    )
    xgb_final.fit(X_full_f, y_full, verbose=False)

    lgb_final = LGBMRegressor(
        n_estimators=int(lgb_best_it) if (lgb_best_it is not None and lgb_best_it > 0) else 2000,
        learning_rate=best_params_b["lgb_learning_rate"],
        num_leaves=best_params_b["lgb_num_leaves"],
        max_depth=best_params_b["lgb_max_depth"],
        min_child_samples=best_params_b["lgb_min_child_samples"],
        subsample=best_params_b["lgb_subsample"],
        colsample_bytree=best_params_b["lgb_colsample_bytree"],
        reg_lambda=best_params_b["lgb_reg_lambda"],
        random_state=SEED, n_jobs=-1, verbosity=-1
    )
    lgb_final.fit(X_full_f, y_full)

    X_te = te_b[feature_cols].fillna(med_full)
    px_log = xgb_final.predict(X_te)
    pl_log = lgb_final.predict(X_te)
    preds = W_XGB * np.clip(np.expm1(px_log), 0, None) + W_LGB * np.clip(np.expm1(pl_log), 0, None)
    test_pred_build[te_b.index] = preds

    build_group_means.append(study_b.best_value)
    metrics_build.append({
        "group": str(bno), "split": "optuna_holdout_sampled",
        "rows": len(tr_b), "val_rows": len(X_val), "val_smape": study_b.best_value, "used_global": 0
    })

# ===== 4-C) Global-Full LGBM (Optuna + sampling 0.1; then refit on ALL) =====
# sampling (0.1) for HPO
global_samp = train_clean.sample(frac=SAMPLING_GLOBAL, random_state=SEED) if len(train_clean) > 0 else train_clean
if 'date' in global_samp.columns and TIME_AWARE:
    global_samp = global_samp.sort_values('date')
    split = int(len(global_samp) * 0.9)
    g_tr, g_val = global_samp.iloc[:split], global_samp.iloc[split:]
else:
    g_tr, g_val = train_test_split(global_samp, test_size=0.1, random_state=SEED, shuffle=True)
...
study_g = _tqdm_optimize(obj_global, N_TRIALS_GLOBAL, desc="global")
best_params_g = study_g.best_params
best_iter_g   = study_g.best_trial.user_attrs.get("lgb_best_iter", None)

# refit on ALL train with best params
...
lgb_full = LGBMRegressor(
    n_estimators=int(best_iter_g) if (best_iter_g is not None and best_iter_g > 0) else 2000,
    learning_rate=best_params_g["lgb_learning_rate"],
    num_leaves=best_params_g["lgb_num_leaves"],
    max_depth=best_params_g["lgb_max_depth"],
    min_child_samples=best_params_g["lgb_min_child_samples"],
    subsample=best_params_g["lgb_subsample"],
    colsample_bytree=best_params_g["lgb_colsample_bytree"],
    reg_lambda=best_params_g["lgb_reg_lambda"],
    random_state=SEED, n_jobs=-1, verbosity=-1
)
...
val_smape_global = study_g.best_value  # HPO holdout proxy
...
print(f"[VAL] type_mean={val_smape_type:.4f}  build_mean={val_smape_build:.4f}  global_holdout={val_smape_global:.4f}  final~={val_smape_final:.4f}")

#endregion

# 6) Save
import datetime
save_path = f'./Energy/02/{SEED}_(ver9)submission/'
...
filename = f"(ver9){SEED}_02_{today}_SMAPE_{score_str}.csv"
samplesub.to_csv(save_path + filename, index=False)

pd.DataFrame(metrics_type ).to_csv(save_path + '(ver9)lgb_by_type_metrics.csv', index=False)
pd.DataFrame(metrics_build).to_csv(save_path + '(ver9)xgb_lgb_by_building_metrics.csv', index=False)

with open(save_path + "(LOG)model.txt", "a") as f:
    f.write(f"<SEED :{SEED}> ver9 Hybrid+Global (Optuna + sampling)\n")
    f.write(f"{filename}\n")
    f.write(f"{drop_cols}\n")
    f.write(f"train file name : {train_path}\n")
    f.write(f"test file name : {test_path}\n")
    f.write(f"Add Data SEED : {addSeed}\n")
    f.write(f"[Type ] mean SMAPE={val_smape_type:.4f} (samp={SAMPLING_TYPE}, trials={N_TRIALS_TYPE})\n")
    f.write(f"[BNO  ] mean SMAPE={val_smape_build:.4f} (samp={SAMPLING_BNO}, trials={N_TRIALS_BNO})\n")
    f.write(f"[Global(HPO holdout)] SMAPE={val_smape_global:.4f} (samp={SAMPLING_GLOBAL}, trials={N_TRIALS_GLOBAL})\n")
