...
seed_file = "./Energy/02/log/(model_ver6)SEED_COUNTS.json"
...
# === Hybrid: LGBM per-building-type + (XGB+LGBM) per-building-number, with final ensemble (SMAPE reporting) ===
import os
import numpy as np
import pandas as pd
...
# External variables assumed to exist: SEED, data_path, train_path, test_path, addSeed
...
# Inner ensemble weights for per-building-number (XGB vs LGBM)
W_XGB, W_LGB = 0.5, 0.5

# Final ensemble weights (type-level vs building-level)
W_FINAL_TYPE, W_FINAL_BUILD = 0.4, 0.6
...
drop_cols = {TARGET, '일시', 'date', '일조(hr)', 'month', TYPE_COL, BNO_COL}  # 누수/상수 방지
print(drop_cols)
...
# 2) 예측/메트릭 버퍼
test_pred_type  = np.zeros(len(test), dtype=float)   # 유형별 LGB
test_pred_build = np.zeros(len(test), dtype=float)   # 건물번호별 XGB+LGB
...
def fit_global_lgb():
    ...
    print(f"[Global LGB] val SMAPE={smape:.3f}  best_it={getattr(lgb,'best_iteration_',None)}")
    return lgb, med
...
def fit_global_xgb():
    ...
    smape = SMAPE(np.expm1(y_val), np.clip(np.expm1(val_log), 0, None))
    print(f"[Global XGB] val SMAPE={smape:.3f}  best_it={best_it}")
    return xgb, med

global_lgb, global_med = fit_global_lgb()
global_xgb, _          = fit_global_xgb()  # med는 동일 컬럼 기준이라 global_med 재사용
...
for t in test[TYPE_COL].unique():
    ...
    if len(tr_type) < max(50, N_SPLITS * 10):
        ...
        preds_log, _ = _predict_lgb(global_lgb, X_te)
        preds = np.clip(np.expm1(preds_log), 0, None)
        test_pred_type[te_type.index] = preds
        metrics_type.append({"group": str(t), "fold": 0, "rows": len(tr_type), "val_rows": 0, "val_smape": np.nan, "used_global": 1})
    ...
    y_full = np.log1p(tr_type[TARGET])

    if TIME_AWARE and 'date' in tr_type.columns:
        splitter = TimeSeriesSplit(n_splits=N_SPLITS)
        splits = list(splitter.split(X_full))
        print("[MODE] TimeSeriesSplit (Type)")
    else:
        splitter = KFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED)
        splits = list(splitter.split(X_full))
        print("[MODE] KFold (Type)")

    fold_smapes = []
    pred_buf = np.zeros(len(te_type), dtype=float)
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits, start=1):
        ...
        print(f"[Type {t}] fold {fold}/{N_SPLITS} rows={len(tr_type):6d}  val SMAPE(LGB)={smape:.3f}  best_it={getattr(lgb,'best_iteration_',None)}")

        metrics_type.append({
            "group": str(t), "fold": fold, "rows": len(tr_type),
        ...
        preds = np.clip(np.expm1(preds_log), 0, None)
        pred_buf += preds / N_SPLITS

    mean_smape = float(np.mean(fold_smapes)) if fold_smapes else np.nan
    type_group_means.append(mean_smape)
    print(f"[Type {t}] mean SMAPE over {N_SPLITS} folds = {mean_smape:.3f}")
    test_pred_type[te_type.index] = pred_buf

# 4-B) 건물번호별(XGB+LGB 앙상블)
for bno in test[BNO_COL].unique():
    tr_b = train_clean[train_clean[BNO_COL] == bno]
    te_b = test[test[BNO_COL] == bno]

    if len(tr_b) < max(50, N_SPLITS * 10):
        X_te = te_b[feature_cols].fillna(global_med)
        px_log, _, _ = _predict_xgb(global_xgb, X_te)
        pl_log, _    = _predict_lgb(global_lgb, X_te)
        preds = W_XGB * np.clip(np.expm1(px_log), 0, None) + W_LGB * np.clip(np.expm1(pl_log), 0, None)
        test_pred_build[te_b.index] = preds
        metrics_build.append({"group": str(bno), "fold": 0, "rows": len(tr_b), "val_rows": 0, "val_smape": np.nan, "used_global": 1})
        print(f"[BNO {bno}] rows={len(tr_b):4d} -> use GLOBAL XGB+LGB")
        continue

    if 'date' in tr_b.columns:
        tr_b = tr_b.sort_values('date')

    X_full = tr_b[feature_cols]
    y_full = np.log1p(tr_b[TARGET])

    if TIME_AWARE and 'date' in tr_b.columns:
        splitter = TimeSeriesSplit(n_splits=N_SPLITS)
        splits = list(splitter.split(X_full))
        print("[MODE] TimeSeriesSplit (BNO)")
    else:
        splitter = KFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED)
        splits = list(splitter.split(X_full))
        print("[MODE] KFold (BNO)")

    fold_smapes = []
    pred_buf = np.zeros(len(te_b), dtype=float)
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits, start=1):
        ...
        print(f"[BNO {bno}] fold {fold}/{N_SPLITS} rows={len(tr_b):6d}  "
              f"val SMAPE(ENS)={smape:.3f}  XGB(best_it={xgb_best})  LGB(best_it={getattr(lgb,'best_iteration_',None)})")

        metrics_build.append({
            "group": str(bno), "fold": fold, "rows": len(tr_b),
            "val_rows": len(X_val), "val_smape": smape, "used_global": 0
        })
    ...
    mean_smape = float(np.mean(fold_smapes)) if fold_smapes else np.nan
    build_group_means.append(mean_smape)
    print(f"[BNO {bno}] mean SMAPE over {N_SPLITS} folds = {mean_smape:.3f}")
    test_pred_build[te_b.index] = pred_buf

# 5) 결과 합치기(최종 앙상블)
test_pred_final = W_FINAL_TYPE * test_pred_type + W_FINAL_BUILD * test_pred_build
...
# 파일명 표기를 위한 가중 평균(참고용)
val_smape_final = np.nan
if not np.isnan(val_smape_type) and not np.isnan(val_smape_build):
    val_smape_final = W_FINAL_TYPE * val_smape_type + W_FINAL_BUILD * val_smape_build
print(f"[VAL] type_mean={val_smape_type:.4f}  build_mean={val_smape_build:.4f}  final~={val_smape_final:.4f}")

# 6) 저장
import datetime
save_path = f'./Energy/02/{SEED}_(ver6)submission/'
...
filename = f"(ver6){SEED}_02_{today}_SMAPE_{score_str}.csv"
samplesub.to_csv(save_path + filename, index=False)

pd.DataFrame(metrics_type ).to_csv(save_path + '(ver6)lgb_by_type_metrics.csv', index=False)
pd.DataFrame(metrics_build).to_csv(save_path + '(ver6)xgb_lgb_by_building_metrics.csv', index=False)

with open(save_path + "(LOG)model.txt", "a") as f:
    f.write(f"<SEED :{SEED}> ver6 Hybrid kfold\n")
    f.write(f"{filename}\n")
    f.write(f"{drop_cols}\n")
    f.write(f"train file name : {train_path}\n")
    f.write(f"test file name : {test_path}\n")
    f.write(f"Add Data SEED : {addSeed}\n")
    f.write(f"[Type]  KFold={N_SPLITS}, TIME_AWARE={TIME_AWARE}, mean SMAPE={val_smape_type:.4f}\n")
    f.write(f"[BNO ]  KFold={N_SPLITS}, TIME_AWARE={TIME_AWARE}, mean SMAPE={val_smape_build:.4f}\n")
    f.write(f"[Final Ensemble] W_TYPE={W_FINAL_TYPE}, W_BUILD={W_FINAL_BUILD}, est SMAPE={val_smape_final}\n")
    f.write("="*40 + "\n")
