...
seed_file = "./Energy/02/log/(model_ver7)SEED_COUNTS.json"
...
# === Hybrid+Global: (Type=LGBM) + (BNO=XGB+LGBM) + (GlobalFull=LGBM), final 3-way ensemble (SMAPE reporting) ===
import os
import numpy as np
import pandas as pd
...
# --- Weights ---
# Inner ensemble for per-building-number
W_XGB, W_LGB = 0.5, 0.5
# Final 3-way ensemble weights (type-level, building-level, global-full)
W_FINAL_TYPE, W_FINAL_BUILD, W_FINAL_GLOBAL = 0.35, 0.45, 0.20
...
for t in test[TYPE_COL].unique():
    ...
    if 'date' in tr_type.columns: tr_type = tr_type.sort_values('date')
    X_full = tr_type[feature_cols]; y_full = np.log1p(tr_type[TARGET])

    if TIME_AWARE and 'date' in tr_type.columns:
        splitter = TimeSeriesSplit(n_splits=N_SPLITS); splits = list(splitter.split(X_full)); print("[MODE] TimeSeriesSplit (Type)")
    else:
        splitter = KFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED); splits = list(splitter.split(X_full)); print("[MODE] KFold (Type)")

    fold_smapes, pred_buf = [], np.zeros(len(te_type), dtype=float)
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits, start=1):
        ...
        metrics_type.append({"group": str(t), "fold": fold, "rows": len(tr_type), "val_rows": len(X_val), "val_smape": smape, "used_global": 0})
    ...
    type_group_means.append(float(np.mean(fold_smapes)) if fold_smapes else np.nan)
    print(f"[Type {t}] mean SMAPE over {N_SPLITS} folds = {type_group_means[-1]:.3f}")
    test_pred_type[te_type.index] = pred_buf

# 4-B) Building-number level (XGB+LGB ensemble)
for bno in test[BNO_COL].unique():
    ...
    if 'date' in tr_b.columns: tr_b = tr_b.sort_values('date')
    X_full = tr_b[feature_cols]; y_full = np.log1p(tr_b[TARGET])

    if TIME_AWARE and 'date' in tr_b.columns:
        splitter = TimeSeriesSplit(n_splits=N_SPLITS); splits = list(splitter.split(X_full)); print("[MODE] TimeSeriesSplit (BNO)")
    else:
        splitter = KFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED); splits = list(splitter.split(X_full)); print("[MODE] KFold (BNO)")

    fold_smapes, pred_buf = [], np.zeros(len(te_b), dtype=float)
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits, start=1):
        ...
        metrics_build.append({"group": str(bno), "fold": fold, "rows": len(tr_b), "val_rows": len(X_val), "val_smape": smape, "used_global": 0})
    ...
    build_group_means.append(float(np.mean(fold_smapes)) if fold_smapes else np.nan)
    print(f"[BNO {bno}] mean SMAPE over {N_SPLITS} folds = {build_group_means[-1]:.3f}")
...
# 6) Save
import datetime
save_path = f'./Energy/02/{SEED}_(ver7)submission/'
...
filename = f"(ver7){SEED}_02_{today}_SMAPE_{score_str}.csv"
samplesub.to_csv(save_path + filename, index=False)

pd.DataFrame(metrics_type ).to_csv(save_path + '(ver7)lgb_by_type_metrics.csv', index=False)
pd.DataFrame(metrics_build).to_csv(save_path + '(ver7)xgb_lgb_by_building_metrics.csv', index=False)



with open(save_path + "(LOG)model.txt", "a") as f:
    f.write(f"<SEED :{SEED}> ver7 Hybrid+Global kfold\n")
    f.write(f"{filename}\n")
    f.write(f"{drop_cols}\n")
