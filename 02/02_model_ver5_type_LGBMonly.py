...
######################################################################################
######################################################################################
# === LGBM per-building-type with KFold (SMAPE reporting) ===
import os
import numpy as np
import pandas as pd
...
def fit_global_backup():
    ...
    val_pred_log, used_iter = _predict_lgb(lgb, X_val)
    y_val_exp = np.expm1(y_val)
    pred = np.clip(np.expm1(val_pred_log), 0, None)

    smape = SMAPE(y_val_exp, pred)
    print(f"[Global] val SMAPE={smape:.3f}  LGB(best_it={getattr(lgb,'best_iteration_',None)})")

    metrics.append({"group": "GLOBAL", "fold": 0, "rows": len(df), "val_rows": len(X_val), "val_smape": smape})
    return (lgb, med)

global_lgb, global_med = fit_global_backup()

all_group_means = []

# 4) 유형별 KFold 학습→예측 (LGBM 단일)
for t in test[GROUP_COL].unique():
    ...
    if len(tr_type) < max(50, N_SPLITS * 10):
        ...
        print(f"[{t}] rows={len(tr_type):4d} -> use GLOBAL LGBM")
        continue
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits, start=1):
        ...
        smape = SMAPE(y_val_exp, val_pred)
        fold_smapes.append(smape)

        print(f"[{t}] fold {fold}/{N_SPLITS}  rows={len(tr_type):6d}  "
              f"val SMAPE(LGB)={smape:.3f}  LGB(best_it={getattr(lgb,'best_iteration_',None)})")
...
metrics_df.to_csv(save_path + '(ver2)lgb_by_type_metrics.csv', index=False)

with open(save_path + "(LOG)model.txt", "a") as f:
    f.write(f"<SEED :{SEED}> kfold (LGBM-only)\n")
    f.write(f"{filename}\n")
    f.write(f"{drop_cols}\n")
