...
drop_cols = {TARGET, '일시', 'date', '일조(hr)', '강수량(mm)', '풍속(m/s)', 'month'}
print(drop_cols)
...
for t in test[GROUP_COL].unique():
    ...
    print(f"[{t}] rows={len(tr_type):6d}  val SMAPE(ENS)={smape:.3f}  "
          f"XGB(best_it={best_iter_xgb})  LGB(best_it={getattr(lgb,'best_iteration_',None)})")
...
filename = f"(ver1){SEED}_02_{today}_SMAPE_{score_str}.csv"
samplesub.to_csv(save_path + filename, index=False)

metrics_df = pd.DataFrame(metrics)
metrics_df.to_csv(save_path + '(ver1)xgb_by_type_metrics.csv', index=False)
