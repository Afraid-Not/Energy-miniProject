...
seed_file = "./Energy/02/log/(model_ver4)SEED_COUNTS.json"
...
######################################################################################
######################################################################################
# === XGBoost per-building-number with KFold (SMAPE reporting) ===
import os
import numpy as np
import pandas as pd
from xgboost import XGBRegressor
...
drop_cols = {TARGET, '일시', 'date', '일조(hr)', 'month', '건물번호'}  # ✅ 건물번호 제외
print(drop_cols)
...
metrics = []   # 각 건물번호/폴드별 val SMAPE 기록
...
# 4) 건물번호별 KFold 학습→예측 (XGB+LGBM 학습 후 앙상블)
for bno in test[GROUP_COL].unique():
    tr_grp = train_clean[train_clean[GROUP_COL] == bno]
    te_grp = test[test[GROUP_COL] == bno]

    # 학습 데이터 부족 시 글로벌 모델 사용
    if len(tr_grp) < max(50, N_SPLITS * 10):
        X_te = te_grp[feature_cols].fillna(global_med)
        ...
        test_pred[te_grp.index] = preds
        metrics.append({"group": str(bno), "fold": 0, "rows": len(tr_grp), "val_rows": 0, "val_smape": np.nan, "used_global": 1})
        print(f"[건물 {bno}] rows={len(tr_grp):4d} -> use GLOBAL ensemble")
        continue

    # 정렬(시계열 안정성)
    if 'date' in tr_grp.columns:
        tr_grp = tr_grp.sort_values('date')

    X_full = tr_grp[feature_cols]
    y_full = np.log1p(tr_grp[TARGET])  # —— 로그 변환

    # 분할자
    if TIME_AWARE and 'date' in tr_grp.columns:
    ...
    # 각 폴드 결과
    fold_smapes = []
    test_pred_grp = np.zeros(len(te_grp), dtype=float)
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits, start=1):
        ...
        print(f"[건물 {bno}] fold {fold}/{N_SPLITS}  rows={len(tr_grp):6d}  "
        ...
        X_te = te_grp[feature_cols].fillna(med)
        preds_xgb_log, _, _ = _predict_xgb(xgb, X_te)
        preds_lgb_log, _ = _predict_lgb(lgb, X_te)
        preds = W_XGB * np.clip(np.expm1(preds_xgb_log), 0, None) + \
                W_LGB * np.clip(np.expm1(preds_lgb_log), 0, None)
        test_pred_grp += preds / N_SPLITS

    # 건물별 폴드 평균 SMAPE
    grp_mean_smape = float(np.mean(fold_smapes)) if fold_smapes else np.nan
    all_group_means.append(grp_mean_smape)
    print(f"[건물 {bno}] mean SMAPE over {N_SPLITS} folds = {grp_mean_smape:.3f}")

    # 건물별 test 예측 저장
    test_pred[te_grp.index] = test_pred_grp

# 전체 평균(SMAPE) - 건물별 폴드 평균의 평균
val_smape = float(np.nanmean(all_group_means)) if len(all_group_means) else np.nan
print(f"[전체 평균] mean SMAPE across buildings = {val_smape:.4f}")

import datetime
save_path = f'./Energy/02/{SEED}_(ver4)submission/'
...
filename = f"(ver4){SEED}_02_{today}_SMAPE_{score_str}.csv"
samplesub.to_csv(save_path + filename, index=False)

metrics_df = pd.DataFrame(metrics)
metrics_df.to_csv(save_path + '(ver4)xgb_by_building_metrics.csv', index=False)  # ✅ 파일명 수정
...
with open(save_path + "(LOG)model.txt", "a") as f:
    ...
    f.write(f"건물번호별 KFold={N_SPLITS}, TIME_AWARE={TIME_AWARE}\n")   # ✅ 로그 문구 수정
    f.write(f"건물번호별 평균 SMAPE: {val_smape:.4f}\n")                 # ✅ 로그 문구 수정
    f.write("="*40 + "\n")
